...
ZIWEI_JI_PP383_12GR_BATCH = ROOT / "docs/FUSION-CHART-HISTORICAL-PROVENANCE-AUDIT-BATCH-12-ZIWEI-JI-SHUYING-WENJI-PP383-385-CHEN-QINGHUA-PURCHASE-CONTEXT-FALSE-LEAD-FIREWALL-GR.md"
...
def main() -> int:
    ...
    try:
        schema12gi = tuple(int(part) for part in state.get("schema_version", "0.0.0").split("."))
    except ValueError:
        fail("Batch 12GI current-state schema version is not numeric")
    if schema12gi < (1, 195, 0):
        fail("Batch 12GI current-state schema version regressed below 1.195.0")
    ...
    # Batch 12GR Ji Wenji pp383-385 Chen Qinghua purchase-context false-lead firewall.
    for path in (ZIWEI_JI_PP383_12GR_BATCH, ZIWEI_JI_PP383_12GR_EVIDENCE):
        if not path.is_file():
            fail(f"Batch 12GR continuity artifact missing: {path.relative_to(ROOT)}")
    batch12gr = json.loads(ZIWEI_JI_PP383_12GR_EVIDENCE.read_text(encoding="utf-8"))
    bid12gr = "BATCH-12-ZIWEI-JI-SHUYING-WENJI-PP383-385-CHEN-QINGHUA-PURCHASE-CONTEXT-FALSE-LEAD-FIREWALL-GR"
    if batch12gr.get("batch_id") != bid12gr:
        fail("Batch 12GR evidence identity mismatch")
    dpm12gr = batch12gr.get("dpm_article_identity", {})
    if dpm12gr.get("provider") != "故宫博物院" or dpm12gr.get("author") != "刘蔷" or dpm12gr.get("printed_page_range") != "53—70":
        fail("Batch 12GR DPM article identity regressed")
    p6212gr = batch12gr.get("printed_page_62_control", {})
    if p6212gr.get("printed_page") != 62 or p6212gr.get("footnote_2_contains_ji_wenji_pages") != "383—385" or p6212gr.get("directly_reviewed_page_image") is not True:
        fail("Batch 12GR p62 citation control regressed")
    p6312gr = batch12gr.get("printed_page_63_control", {})
    if p6312gr.get("collector") != "陈清华" or p6312gr.get("state_acquisition_years") != [1956, 1965] or p6312gr.get("context_identity") != "CHEN_QINGHUA_BULK_PURCHASE_DISCUSSION":
        fail("Batch 12GR p63 referent/context regressed")
    adj12gr = batch12gr.get("adjudication", {})
    if adj12gr.get("liu_2010_use_of_ji_pp383_385") != "CHEN_QINGHUA_BULK_PURCHASE_CONTEXT" or adj12gr.get("direct_ji_pp383_385_text") != "NOT_REVIEWED":
        fail("Batch 12GR citation/direct-page boundary regressed")
    if adj12gr.get("ji_pp383_385_as_baichuan_page_locator") != "NOT_AUTHORIZED" or adj12gr.get("ji_pp383_385_as_tieqin_chapter_page_locator") != "NOT_AUTHORIZED" or adj12gr.get("ji_pp383_385_proven_absent_of_tieqin_material") is not False:
        fail("Batch 12GR false-lead firewall regressed")
    if adj12gr.get("target_specific_purchase_route_selected") is not False or adj12gr.get("target_specific_donation_route_selected") is not False:
        fail("Batch 12GR target transfer route falsely selected")
    srcids12gr = {x.get("source_id"): x for x in registry.get("sources", ())}
    if "EXT-DPM-LIU-QIANG-2010-TIANLU-PP62-63-JI383-385-CONTEXT" not in srcids12gr:
        fail("Batch 12GR source registry missing")
    trans12gr = batch12gr.get("transmission_impact", {})
    if trans12gr.get("nodes_added") != [] or trans12gr.get("edges_added") != [] or trans12gr.get("direct_copy_edge_authorized") is not False or trans12gr.get("same_object_edge_authorized") is not False or trans12gr.get("acquisition_edge_authorized") is not False:
        fail("Batch 12GR topology/genealogy firewall regressed")
    rule12gr = batch12gr.get("chronology_and_rule_firewall", {})
    if rule12gr.get("runtime_rule_change") is not False or rule12gr.get("algorithm_reopen_authorized") is not False or rule12gr.get("candidate_collapse_authorized") is not False or rule12gr.get("matrix_count_change") is not False:
        fail("Batch 12GR product/matrix firewall regressed")
    acct12gr = batch12gr.get("accounting", {})
    if acct12gr.get("matrix_rows") != 198 or acct12gr.get("audited_rows") != 166 or acct12gr.get("current_missing_from_product_rows") != 10:
        fail("Batch 12GR matrix accounting unexpectedly changed")
    if acct12gr.get("confirmed_provenance_metadata_defect_count") != 14 or acct12gr.get("repaired_provenance_metadata_defect_count") != 14 or acct12gr.get("confirmed_chart_algorithm_defect_count") != 0:
        fail("Batch 12GR provenance/algorithm accounting regressed")
    try:
        schema12gr = tuple(int(part) for part in state.get("schema_version", "0.0.0").split("."))
    except ValueError:
        fail("Batch 12GR current-state schema version is not numeric")
    if schema12gr < (1, 204, 0):
        fail("Batch 12GR current-state schema version regressed below 1.204.0")
    if bid12gr not in audit_state.get("completed_batches", ()):
        fail("Batch 12GR missing from completed batch state")
    if audit_state.get("latest_batch_doc") != LATEST_BATCH_DOC:
        fail("Batch 12GR latest-batch document state mismatch")

    # Batch 12GS Ji Wenji 2004 public full-TOC item-74/editorial-endmatter boundary.
    for path in (ZIWEI_JI_TOC_12GS_BATCH, ZIWEI_JI_TOC_12GS_EVIDENCE):
        if not path.is_file():
            fail(f"Batch 12GS continuity artifact missing: {path.relative_to(ROOT)}")
    batch12gs = json.loads(ZIWEI_JI_TOC_12GS_EVIDENCE.read_text(encoding="utf-8"))
    bid12gs = "BATCH-12-ZIWEI-JI-SHUYING-WENJI-2004-PUBLIC-FULL-TOC-ARTICLE74-EDITORIAL-ENDMATTER-BOUNDARY-GS"
    if batch12gs.get("batch_id") != bid12gs:
        fail("Batch 12GS evidence identity mismatch")
    toc12gs = batch12gs.get("public_full_toc_control", {})
    target12gs = toc12gs.get("target_entry", {})
    if toc12gs.get("complete_toc_sequence_exposed") is not True or target12gs.get("ordinal") != 74 or target12gs.get("normalized_title") != "百川归海，蔚为大观":
        fail("Batch 12GS target TOC locator regressed")
    succ12gs = toc12gs.get("immediate_successor", {})
    if succ12gs.get("ordinal") != 75 or succ12gs.get("normalized_for_locator_only") != "中国古代目录学简述":
        fail("Batch 12GS successor locator regressed")
    if toc12gs.get("appendix_entries_displayed") != 3 or toc12gs.get("final_endmatter_display") != "編后記":
        fail("Batch 12GS endmatter structure regressed")
    if toc12gs.get("target_article_page_numbers_exposed") is not False or toc12gs.get("target_article_body_exposed") is not False or toc12gs.get("final_endmatter_body_exposed") is not False:
        fail("Batch 12GS access boundary regressed")
    adj12gs = batch12gs.get("adjudication", {})
    if adj12gs.get("baichuan_toc_ordinal") != "CLOSED_AT_PUBLIC_FULL_TOC_LAYER_AS_ITEM_74" or adj12gs.get("baichuan_exact_page_range") != "UNRESOLVED" or adj12gs.get("direct_2004_article_text") != "NOT_REVIEWED":
        fail("Batch 12GS article locator/direct-text boundary regressed")
    if adj12gs.get("final_editor_postscript_presence") != "CLOSED_AT_TOC_LABEL_LAYER" or adj12gs.get("direct_final_editor_postscript_text") != "NOT_REVIEWED" or adj12gs.get("displayed_bianhouji_equals_secondary_reported_bianzhezhu") != "NOT_PROVED":
        fail("Batch 12GS editor-endmatter firewall regressed")
    if adj12gs.get("retail_p456_as_canonical_pagination") != "NOT_AUTHORIZED" or adj12gs.get("derivation_status") != "SECONDARY_REPORTED_NOT_DIRECTLY_CLOSED":
        fail("Batch 12GS pagination/genealogy firewall regressed")
    if adj12gs.get("target_specific_purchase_route_selected") is not False or adj12gs.get("target_specific_donation_route_selected") is not False:
        fail("Batch 12GS target transfer route falsely selected")
    srcids12gs = {x.get("source_id"): x for x in registry.get("sources", ())}
    if "EXT-FRELAX-SHUCHONG-JI-SHUYING-WENJI-2004-FULL-TOC" not in srcids12gs:
        fail("Batch 12GS source registry missing")
    trans12gs = batch12gs.get("transmission_impact", {})
    if trans12gs.get("nodes_added") != [] or trans12gs.get("edges_added") != [] or trans12gs.get("direct_copy_edge_authorized") is not False or trans12gs.get("same_object_edge_authorized") is not False or trans12gs.get("acquisition_edge_authorized") is not False:
        fail("Batch 12GS topology/genealogy firewall regressed")
    rule12gs = batch12gs.get("chronology_and_rule_firewall", {})
    if rule12gs.get("runtime_rule_change") is not False or rule12gs.get("algorithm_reopen_authorized") is not False or rule12gs.get("candidate_collapse_authorized") is not False or rule12gs.get("matrix_count_change") is not False:
        fail("Batch 12GS product/matrix firewall regressed")
    acct12gs = batch12gs.get("accounting", {})
    if acct12gs.get("matrix_rows") != 198 or acct12gs.get("audited_rows") != 166 or acct12gs.get("current_missing_from_product_rows") != 10:
        fail("Batch 12GS matrix accounting unexpectedly changed")
    if acct12gs.get("confirmed_provenance_metadata_defect_count") != 14 or acct12gs.get("repaired_provenance_metadata_defect_count") != 14 or acct12gs.get("confirmed_chart_algorithm_defect_count") != 0:
        fail("Batch 12GS provenance/algorithm accounting regressed")
    try:
        schema12gs = tuple(int(part) for part in state.get("schema_version", "0.0.0").split("."))
    except ValueError:
        fail("Batch 12GS current-state schema version is not numeric")
    if schema12gs < (1, 205, 0):
        fail("Batch 12GS current-state schema version regressed below 1.205.0")
    if bid12gs not in audit_state.get("completed_batches", ()):
    ...
    # Batch 12GU Wenjin Xuezhi v3 exact Japanese holdings and OPAC access boundary.
    for path in (ZIWEI_WENJIN_V3_12GU_BATCH, ZIWEI_WENJIN_V3_12GU_EVIDENCE):
        if not path.is_file():
            fail(f"Batch 12GU continuity artifact missing: {path.relative_to(ROOT)}")
    batch12gu = json.loads(ZIWEI_WENJIN_V3_12GU_EVIDENCE.read_text(encoding="utf-8"))
    bid12gu = "BATCH-12-ZIWEI-WENJIN-XUEZHI-V3-JAPAN-PHYSICAL-HOLDINGS-AND-OPAC-ACCESS-BOUNDARY-GU"
    if batch12gu.get("batch_id") != bid12gu:
        fail("Batch 12GU evidence identity mismatch")
    c12gu = batch12gu.get("cinii_serial_control", {})
    if c12gu.get("ncid") != "BA65589403" or c12gu.get("target_volume_number") != 3 or c12gu.get("target_isbn_13") != "9787501340859":
        fail("Batch 12GU serial/volume identity regressed")
    hs12gu = batch12gu.get("exact_volume3_holdings", [])
    if len(hs12gu) != 3:
        fail("Batch 12GU exact holding count regressed")
    keys12gu={(x.get("institution"),x.get("call_number"),x.get("holding_or_item_identifier")) for x in hs12gu}
    expected12gu={
        ("京都大学 人文科学研究所 図書室","020.22||Ko/43||3","200035254509"),
        ("京都大学 文学研究科 図書館","FXIIb||B||7","200019220216"),
        ("東京大学 大学院人文社会系研究科・文学部 図書室","3号館B37:Guo:3","4818863757"),
    }
    if keys12gu != expected12gu:
        fail("Batch 12GU exact holdings regressed")
    access12gu=batch12gu.get("access_adjudication", {})
    if access12gu.get("exact_japan_volume3_physical_holding_routes") != "CLOSED_FOR_THREE_INSTITUTIONAL_COPIES_AT_CINII_HOLDING_LAYER":
        fail("Batch 12GU physical-holding adjudication regressed")
    if access12gu.get("supplement_article_author") != "UNRESOLVED" or access12gu.get("supplement_article_page_range") != "UNRESOLVED" or access12gu.get("supplement_article_body") != "NOT_REVIEWED":
        fail("Batch 12GU article-level access boundary regressed")
    if access12gu.get("opac_403_means_library_human_access_denied") is not False or access12gu.get("opac_403_means_reference_or_copy_service_unavailable") is not False:
        fail("Batch 12GU crawler-403 inference firewall regressed")
    if any(access12gu.get(k) for k in ("project_submitted_reference_request","project_submitted_copy_request","project_logged_into_library_account","project_transmitted_user_identity","project_incurred_fee")):
        fail("Batch 12GU external-action firewall violated")
    if access12gu.get("external_action_requires_explicit_user_authorization") is not True:
        fail("Batch 12GU authorization boundary regressed")
    src12gu=next((x for x in registry.get("sources",()) if x.get("source_id")=="EXT-CINII-WENJIN-XUEZHI-SERIAL-V3-ISBN9787501340859"),None)
    if src12gu is None or src12gu.get("batch_12gu",{}).get("exact_physical_holding_route_count") != 3:
        fail("Batch 12GU registry enrichment missing")
    trans12gu=batch12gu.get("transmission_impact",{})
    if trans12gu.get("nodes_added") != [] or trans12gu.get("edges_added") != [] or trans12gu.get("direct_copy_edge_authorized") is not False or trans12gu.get("same_object_edge_authorized") is not False or trans12gu.get("acquisition_edge_authorized") is not False:
        fail("Batch 12GU genealogy firewall regressed")
    rule12gu=batch12gu.get("chronology_and_rule_firewall",{})
    if rule12gu.get("runtime_rule_change") is not False or rule12gu.get("algorithm_reopen_authorized") is not False or rule12gu.get("candidate_collapse_authorized") is not False or rule12gu.get("matrix_count_change") is not False:
        fail("Batch 12GU product/matrix firewall regressed")
    acct12gu=batch12gu.get("accounting",{})
    if acct12gu.get("matrix_rows") != 198 or acct12gu.get("audited_rows") != 166 or acct12gu.get("current_missing_from_product_rows") != 10:
        fail("Batch 12GU matrix accounting unexpectedly changed")
    if acct12gu.get("confirmed_provenance_metadata_defect_count") != 14 or acct12gu.get("repaired_provenance_metadata_defect_count") != 14 or acct12gu.get("confirmed_chart_algorithm_defect_count") != 0:
        fail("Batch 12GU provenance/algorithm accounting regressed")
    try:
        schema12gu=tuple(int(part) for part in state.get("schema_version","0.0.0").split("."))
    except ValueError:
        fail("Batch 12GU current-state schema version is not numeric")
    if schema12gu < (1,207,0):
        fail("Batch 12GU current-state schema version regressed below 1.207.0")
    if bid12gu not in audit_state.get("completed_batches",()):
    ...
    # Batch 12GX Wenjin Xuezhi v3 NLC Press first-party download-surface boundary.
    for path in (ZIWEI_WENJIN_NLCPRESS_12GX_BATCH, ZIWEI_WENJIN_NLCPRESS_12GX_EVIDENCE):
        if not path.is_file():
            fail(f"Batch 12GX continuity artifact missing: {path.relative_to(ROOT)}")
    batch12gx=json.loads(ZIWEI_WENJIN_NLCPRESS_12GX_EVIDENCE.read_text(encoding="utf-8"))
    bid12gx="BATCH-12-ZIWEI-WENJIN-XUEZHI-V3-NLCPRESS-OFFICIAL-DOWNLOAD-SURFACE-BOUNDARY-GX"
    if batch12gx.get("batch_id") != bid12gx:
        fail("Batch 12GX evidence identity mismatch")
    p12gx=batch12gx.get("official_product_control",{})
    if p12gx.get("product_id") != 4396 or p12gx.get("isbn_display") != "978-7-5013-4085-9" or p12gx.get("publication_date") != "2010-05-25":
        fail("Batch 12GX product identity regressed")
    if p12gx.get("compiler_display") != "国家图书馆善本特藏部" or p12gx.get("first_party_page_directly_reviewed") is not True:
        fail("Batch 12GX first-party product control regressed")
    d12gx=batch12gx.get("official_related_downloads",{})
    if d12gx.get("labels_directly_exposed") != ["图书文件下载（TXT）","目录附件下载"] or d12gx.get("official_download_surface_exists") is not True:
        fail("Batch 12GX official download labels regressed")
    if d12gx.get("underlying_attachment_urls_exposed_in_reviewed_text_interface") is not False or d12gx.get("attachment_bytes_recovered") is not False:
        fail("Batch 12GX attachment-access boundary regressed")
    a12gx=batch12gx.get("adjudication",{})
    if a12gx.get("exact_v3_publisher_product_identity") != "CLOSED_FIRST_PARTY_NLCPRESS_PRODUCT_ID_4396":
        fail("Batch 12GX exact publisher object regressed")
    if a12gx.get("supplement_article_author") != "UNRESOLVED" or a12gx.get("supplement_article_page_range") != "UNRESOLVED" or a12gx.get("supplement_article_body") != "NOT_REVIEWED":
        fail("Batch 12GX article-level boundary regressed")
    access12gx=batch12gx.get("access_boundary",{})
    if any(access12gx.get(k) for k in ("guessed_download_endpoint_attempted","endpoint_enumeration_attempted","login_attempted","token_or_cookie_reuse_attempted","account_or_identity_transmitted","fee_incurred")):
        fail("Batch 12GX access firewall violated")
    src12gx=next((x for x in registry.get("sources",()) if x.get("source_id")=="EXT-NLCPRESS-WENJIN-XUEZHI-V3-PRODUCT-DOWNLOAD-SURFACE"),None)
    if src12gx is None or src12gx.get("batch_12gx",{}).get("product_id") != 4396:
        fail("Batch 12GX source registry binding missing")
    trans12gx=batch12gx.get("transmission_impact",{})
    if trans12gx.get("nodes_added") != [] or trans12gx.get("edges_added") != [] or trans12gx.get("direct_copy_edge_authorized") is not False or trans12gx.get("same_object_edge_authorized") is not False or trans12gx.get("acquisition_edge_authorized") is not False:
        fail("Batch 12GX genealogy firewall regressed")
    rule12gx=batch12gx.get("chronology_and_rule_firewall",{})
    if rule12gx.get("runtime_rule_change") is not False or rule12gx.get("algorithm_reopen_authorized") is not False or rule12gx.get("candidate_collapse_authorized") is not False or rule12gx.get("matrix_count_change") is not False:
        fail("Batch 12GX product/matrix firewall regressed")
    acct12gx=batch12gx.get("accounting",{})
    if acct12gx.get("matrix_rows") != 198 or acct12gx.get("audited_rows") != 166 or acct12gx.get("current_missing_from_product_rows") != 10 or acct12gx.get("confirmed_chart_algorithm_defect_count") != 0:
        fail("Batch 12GX accounting regressed")
    try:
        schema12gx=tuple(int(part) for part in state.get("schema_version","0.0.0").split("."))
    except ValueError:
        fail("Batch 12GX current-state schema version is not numeric")
    if schema12gx < (1,210,0):
        fail("Batch 12GX current-state schema version regressed below 1.210.0")
    if bid12gx not in audit_state.get("completed_batches",()):
