...
def main() -> int:
    ...
    if ev12ao.get("batch_id") != "BATCH-12-ZIWEI-JIELAN-INCLEMENT-TIME-ACQUISITION-AO":
    ...
    if audit_summary.get("confirmed_provenance_metadata_defect_count", 0) < 13:
        raise SystemExit("known provenance metadata defects are missing")
    if audit_summary.get("repaired_provenance_metadata_defect_count", 0) < 13:
