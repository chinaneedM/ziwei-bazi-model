...
def main():
    ...
    try:
        rr=s.post(RENDERER,data={"item_cd":ITEM_CD,"book_cd":BOOK_CD,"vol_no":VOL_NO,"page_no":page,"tool":"1"},timeout=20)
        result["renderer_transport"]=meta(rr)
        ...
        if path:
            ...
            ir=s.get(url,headers={"Referer":RENDERER,"Accept":"image/jpeg,image/*,*/*;q=0.8"},timeout=25)
            result["image_transport"]=meta(ir)
