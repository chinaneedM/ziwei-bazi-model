...
def _rolling_blind_metrics(cycle: dict[str, Any], current_first_blind: dict[str, Any]) -> dict[str, Any]:
    ...
    question_id = str(current_first_blind["question_id"])
    rows = [v for v in rows if str(v.get("question_id")) != question_id]
    rows.append({
    ...
    distinct = len({str(v["question_id"]) for v in rows})
...
def evaluate_question_training(
    ...
    contamination = any(
        reason in {"ANSWER_VISIBILITY_NOT_DENIED", "ANSWER_FREE_INPUT_NOT_PROVEN", "CASE_SPECIFIC_RULE_DETECTED", "ANSWER_MEMORIZATION_RULE_DETECTED"}
    ...
    provenance_invalid = any(
        reason in {"SOURCE_PROVENANCE_NOT_PASS", "PAIRWISE_REPLAY_NOT_PASS", "FIRST_BLIND_FREEZE_NOT_PROVEN"}
        for reason in [*first_reasons, *(r for item in replay_rejections for r in item["reasons"])]
    )
