...
from fortune_training.bazi_classical_resolution_failure_effect_disposition.release import (
    CONTRACT_PATH as UNIT9_CONTRACT_PATH,
    validate_release_contract as validate_unit9_release_contract,
)
...
AUDIT_ID = "BAZI-CLASSICAL-CANDIDATE-PRESERVING-REVERSAL-OR-REAPPEARANCE-EFFECT-DISPOSITION-R1"
AUDIT_ROOT = Path("audits/bazi-classical-reversal-reappearance-effect-disposition-r1")
...
EXPECTED_UNIT9_CONTRACT_SEMANTICS_SHA256 = (
...
def build_release_contract(root: Path) -> dict[str, Any]:
    ...
    unit9 = validate_unit9_release_contract(root)
    ...
    if unit9["contract_semantics_sha256"] != EXPECTED_UNIT9_CONTRACT_SEMANTICS_SHA256:
        raise TrainingError("released Unit 9 contract semantic identity changed")

    profile = bazi_classical_reversal_reappearance_effect_disposition_r1_profile()
    contract = {
        "schema": "BAZI-CLASSICAL-REVERSAL-REAPPEARANCE-EFFECT-DISPOSITION-CONTRACT-R1",
        "audit_id": AUDIT_ID,
        "authority": {
            ...
            "unit9_contract_path": UNIT9_CONTRACT_PATH.as_posix(),
            "unit9_contract_semantics_sha256": unit9["contract_semantics_sha256"],
            "unit9_runtime_output_dependency": "NONE_SIBLING_RELEASE_IDENTITY_ONLY",
        },
        "reversal_reappearance_effect_profile": json_value(profile),
        "closed_vocabularies": {
        ...
        "local_reversal_reappearance_closure_contract": {
            "closure_requirement_id": (
                profile.reversal_reappearance_closure_requirement_id
            ),
            "expected_upstream_status": profile.expected_upstream_closure_status,
            "unit10_local_closure_result": profile.local_closure_result,
            "semantic_scope": profile.disposition_semantic_scope,
            "mutates_unit5_unit7_unit8_or_unit9": False,
            "interaction_chain_execution_released": False,
            "global_relation_restored_state_released": False,
            "global_relation_active_in_force_state_released": False,
            "general_resolver_readiness": "NOT_RELEASED",
        },
        "candidate_projection_contract": {
            "unit7_candidate_to_unit10_projection": "EXACTLY_ONE_TO_ONE",
            "handled_reversal_reappearance_candidate_to_disposition": (
                "EXACTLY_ONE_TO_ONE"
            ),
            "non_reversal_reappearance_candidate_disposition_count": 0,
        ...
        "preservation_contract": {
            "unit7_outer_to_unit10_outer": "EXACTLY_ONE_TO_ONE",
            "unit7_fragment_to_unit10_fragment": "EXACTLY_ONE_TO_ONE",
            ...
            "unit8_resolution_runtime_precondition": "FORBIDDEN",
            "unit9_resolution_failure_runtime_precondition": "FORBIDDEN",
            "source_assertion_subtype_inference": "FORBIDDEN",
        ...
        "unreleased_neighbor_primitives": {
            ...
            "COMPATIBLE_EXACT_INSTANCE_PATH_ENUMERATION": "NOT_UPGRADED_BY_UNIT10",
        },
        "hard_exclusions": [
            "GENERAL_CLASSICAL_RESOLVER",
            "EXECUTE_UNIT8_RESOLUTION_BEFORE_UNIT10",
            "EXECUTE_UNIT9_RESOLUTION_FAILURE_BEFORE_UNIT10",
            "SOURCE_NARRATIVE_CHAIN_EXECUTION",
            "SOURCE_ASSERTION_REVERSAL_VS_REAPPEARANCE_SUBTYPE_INFERENCE",
            "TARGET_RELATION_GLOBALLY_RESTORED_ACTIVE_OR_IN_FORCE_STATE",
...
def build_coverage_report(contract: dict[str, Any]) -> str:
    ...
    return "\n".join((
        "# Bazi Classical Candidate-Preserving Reversal-or-Reappearance Effect Disposition R1",
        "",
        "- Unit 10 closes only `CLASSICAL_REVERSAL_OR_REAPPEARANCE_SEMANTICS` as a candidate-local source-asserted reversal-or-reappearance effect disposition.",
        "- The combined source class remains combined; Unit 10 does not infer a reversal-versus-reappearance subtype and does not promote source wording such as `复冲` or `复刑` to global restored/active state.",
        "- Every Unit 7 candidate is preserved exactly once; only reversal/reappearance candidates emit one Unit 10 disposition.",
        "- Units 8 and 9 remain independently released siblings; Unit 10 never executes their runtime outputs as preconditions.",
        "- Interaction-chain execution, attenuation, participant allocation, arbitration, readiness, global relation/effect state and final verdict remain unreleased.",
        f"- Deterministic Unit 10 contract semantics: `{digest}`.",
        "",
    ))
...
def validate_release_contract(root: Path) -> dict[str, Any]:
    ...
    if actual != expected:
        raise TrainingError(
            "Unit 10 reversal/reappearance effect disposition release contract is stale or tampered"
        )
    ...
    if errors:
        raise TrainingError(
            f"Unit 10 release contract schema validation failed: {errors[0].message}"
        )
    ...
    if (
        ...
        raise TrainingError("Unit 10 coverage report is stale or missing")
    return {
        "status": "PASS",
        "audit_id": AUDIT_ID,
...
def profile_value(contract: dict[str, Any], key: str) -> Any:
    return contract["reversal_reappearance_effect_profile"][key]
