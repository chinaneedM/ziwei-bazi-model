...
from .generation import build_branch_relation_positional_context
...
class BaziBranchRelationPositionalGenerationError(ValueError):
...
class BaziBranchRelationPositionalRequest:
    ...
    positional_profile: ResolvedBaziBranchRelationPositionalProfile
...
def _source_structural_candidate(
    ...
    if (
        ...
        raise BaziBranchRelationPositionalGenerationError("STRUCTURAL_LINEAGE_INDEX_INVALID", str(indices))
    ...
    for index, row in zip(indices, rows, strict=True):
        if row.integrity.status != "PASS":
            raise BaziBranchRelationPositionalGenerationError("UPSTREAM_STRUCTURAL_INTEGRITY_FAILED", str(index))
        if row.hashes.fact_hash != source_snapshot.upstream_structural_fact_hash or row.hashes.computation_hash != source_snapshot.upstream_structural_computation_hash:
            raise BaziBranchRelationPositionalGenerationError("STRUCTURAL_HASH_LINEAGE_MISMATCH", str(index))
        if row.hashes != first.hashes or row.context != first.context:
            raise BaziBranchRelationPositionalGenerationError("STRUCTURAL_COMPLETE_CONTRACT_MISMATCH", str(index))
    return first
...
def _group_incidence_candidates(
    ...
    if not candidates:
        raise BaziBranchRelationPositionalGenerationError("NO_INCIDENCE_CANDIDATES", "incidence")
    grouped: dict[tuple[str, str], dict[str, Any]] = {}
    for index, candidate in enumerate(candidates):
        if candidate.integrity.status != "PASS":
            raise BaziBranchRelationPositionalGenerationError("UPSTREAM_INCIDENCE_INTEGRITY_FAILED", str(index))
        ...
        if existing is None:
            ...
            if existing["candidate"] != candidate:
                raise BaziBranchRelationPositionalGenerationError("INCIDENCE_COMPLETE_CONTRACT_COLLISION", candidate.hashes.fact_hash)
...
class BaziBranchRelationPositionalEngine:
    schema = "BAZI-BRANCH-RELATION-POSITIONAL-CONTEXT-FOUNDATION-RESULT-V1"
    typed_schema = "BAZI-BRANCH-RELATION-POSITIONAL-TYPED-RESOLUTION-V1"
    ...
    def resolve_typed(
        ...
        try:
            ...
            return BaziBranchRelationPositionalResolution(self.typed_schema, "FAILED", (), (), (f"PROFILE_INVALID:{exc}",))
        ...
        try:
            for group in _group_incidence_candidates(request.incidence_candidates):
                ...
                if incidence.context.snapshot.upstream_natal_fact_hash != request.natal_candidate.hashes.fact_hash:
                    raise BaziBranchRelationPositionalGenerationError("INCIDENCE_NATAL_LINEAGE_MISMATCH", incidence.context.snapshot.upstream_natal_fact_hash)
                structural = _source_structural_candidate(incidence, request.structural_candidates)
                context = build_branch_relation_positional_context(request.natal_candidate, structural, incidence, request.positional_profile)
                binding_keys = _lineage_binding_keys(incidence, incidence_indices)
                hashes = branch_relation_positional_hash_bundle(
                    context,
                    incidence,
                    incidence_indices,
                ...
                integrity = validate_branch_relation_positional_context(
                    context,
                ...
                if integrity.status != "PASS":
                    return BaziBranchRelationPositionalResolution(
                        self.typed_schema,
                        "FAILED",
                        (),
                        (),
                        tuple(f"INTEGRITY:{row.code}:{row.path}" for row in integrity.diagnostics),
                    )
                rows.append(BaziBranchRelationPositionalCandidate(
        ...
        except (BaziBranchRelationPositionalGenerationError, ValueError, KeyError) as exc:
            code = getattr(exc, "diagnostic_code", "POSITIONAL_GENERATION_FAILED")
            return BaziBranchRelationPositionalResolution(self.typed_schema, "FAILED", (), (), (f"{code}:{exc}",))
        ...
        return BaziBranchRelationPositionalResolution(
            ...
            events=("AMBIGUOUS_LINEAGE_CANDIDATES_PRESERVED",) if len(candidates) > 1 else (),
            diagnostics=(),
        )

    def resolve(self, request: BaziBranchRelationPositionalRequest) -> dict[str, Any]:
