...
(() => {
  ...
  function resetRenderedResolution(){ last=null; $('manifest-hash').textContent='-'; $('manifest-hash').removeAttribute('title'); $('ziwei-status').textContent='-'; $('ziwei-hash').textContent='-'; $('bazi-status').textContent='-'; $('bazi-hash').textContent='-'; showSubError('ziwei-error',null); showSubError('bazi-error',null); const zroot=$('ziwei-chart'); clear(zroot); zroot.className='placeholder'; zroot.textContent='当前请求未产生紫微结果'; const broot=$('bazi-chart'); clear(broot); broot.className='placeholder'; broot.textContent='当前请求未产生八字结果'; ['download-manifest','download-ziwei','download-bazi'].forEach((id)=>$(id).disabled=true); }
