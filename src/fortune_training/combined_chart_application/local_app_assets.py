...
(() => {
  ...
  function renderShensha(root,set){ if(!set?.candidates?.length)return; const wrap=node('section',undefined,'shensha'); wrap.append(node('div','神煞事实候选（年柱法 / 日柱法分列，不裁决、不合并）','shensha-head')); const items=node('div',undefined,'shensha-items'); const basis={DAY_STEM:'日干',YEAR_STEM:'年干',DAY_BRANCH:'日支',YEAR_BRANCH:'年支'}; set.candidates.filter((row)=>row.present).forEach((row)=>{const positions=row.occurrences.map((hit)=>hit.pillar_position).join('、'); items.append(node('span',`${row.display_name}｜${basis[row.anchor_basis]||row.anchor_basis} ${row.anchor_value} → ${positions}`,'shensha-item'));}); if(!items.childElementCount)items.append(node('span','当前四柱无匹配','shensha-item')); wrap.append(items); root.append(wrap); }
