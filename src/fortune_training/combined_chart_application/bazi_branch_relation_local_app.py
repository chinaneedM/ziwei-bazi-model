...
BAZI_BRANCH_RELATION_PRESENTATION_ENDPOINT = "/api/bazi-branch-relations-presentation"
BAZI_BRANCH_RELATION_PRESENTATION_SCHEMA = (
    "COMBINED-BAZI-BRANCH-RELATION-PRESENTATION-R1"
)
_BRANCH_RELATION_PREFIX = "BRANCH_"


def _branch_relation_rows(chart: Any) -> list[dict[str, Any]]:
    branches = {branch.instance_id: branch for branch in chart.branches}
    rows: list[dict[str, Any]] = []
    for relation in chart.raw_relations:
        if not relation.relation_family.startswith(_BRANCH_RELATION_PREFIX):
            continue
        ...
        for instance_id in relation.participant_instance_ids:
            ...
            if branch is None:
                raise ValueError(
                    f"relation {relation.relation_id} references unknown branch {instance_id}"
                )
            participants.append(
                {
...
class BaziBranchRelationPresentationLocalMixin:
    """Read-only natal branch-relation facts over the released Ba Zi foundation."""

    def resolve_bazi_branch_relation_presentation_payload(
        self,
        ...
        try:
            ...
            raise LocalCombinedAppRequestError(
                "LOCAL_APP_BAZI_BRANCH_RELATION_SOURCE_RESOLUTION_FAILED",
                str(exc),
                status=422,
            ) from exc
        ...
        if bazi_bundle is None:
            ...
            raise LocalCombinedAppRequestError(
                ...
                else "LOCAL_APP_BAZI_BRANCH_RELATION_BAZI_MISSING",
                error.detail
                if error is not None
        ...
        if natal_resolution.status == "FAILED" or not natal_resolution.candidates:
            raise LocalCombinedAppRequestError(
                "LOCAL_APP_BAZI_BRANCH_RELATION_NATAL_REPLAY_FAILED",
        ...
        for application_index, application_candidate in enumerate(bazi_bundle.candidates):
            ...
            if natal_index < 0 or natal_index >= len(natal_resolution.candidates):
                raise LocalCombinedAppRequestError(
                    "LOCAL_APP_BAZI_BRANCH_RELATION_NATAL_INDEX_MISMATCH",
                    (
            ...
            if natal_candidate.hashes.fact_hash != application_candidate.natal_fact_hash:
                raise LocalCombinedAppRequestError(
                    "LOCAL_APP_BAZI_BRANCH_RELATION_NATAL_FACT_HASH_MISMATCH",
                    f"application_candidate_index={application_index}",
                    status=422,
                )
            if (
                ...
                raise LocalCombinedAppRequestError(
                    "LOCAL_APP_BAZI_BRANCH_RELATION_NATAL_COMPUTATION_HASH_MISMATCH",
                    f"application_candidate_index={application_index}",
                    status=422,
                )
            try:
                relations = _branch_relation_rows(natal_candidate.chart)
            except ValueError as exc:
                raise LocalCombinedAppRequestError(
                    "LOCAL_APP_BAZI_BRANCH_RELATION_IDENTITY_INVALID",
                    str(exc),
                    status=422,
                ) from exc
        ...
        return {
            "schema": BAZI_BRANCH_RELATION_PRESENTATION_SCHEMA,
...
class _BaziBranchRelationPresentationHandlerMixin(_Handler):
    application: BaziBranchRelationPresentationLocalMixin

    def do_POST(self) -> None:  # noqa: N802
        if urlsplit(self.path).path != BAZI_BRANCH_RELATION_PRESENTATION_ENDPOINT:
            super().do_POST()
            return
        if (
        ...
        try:
            ...
            return
        try:
            response = self.application.resolve_bazi_branch_relation_presentation_payload(
                payload
            )
