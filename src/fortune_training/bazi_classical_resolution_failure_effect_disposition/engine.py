...
class BaziClassicalResolutionFailureEffectDispositionError(ValueError):
...
class BaziClassicalResolutionFailureEffectDispositionRequest:
    ...
    resolution_failure_effect_profile: ClassicalResolutionFailureEffectDispositionProfile
...
def _project_envelope(
    ...
    hashes = resolution_failure_effect_hash_bundle(
        source_final_envelope,
        fragment_projections,
        source_sets,
        effect_index,
    ...
    integrity = validate_resolution_failure_effect_envelope(
        source_final_envelope,
        fragment_projections,
        source_sets,
        effect_index,
    ...
    if integrity.status != "PASS":
        raise BaziClassicalResolutionFailureEffectDispositionError(
            "RESOLUTION_FAILURE_EFFECT_DISPOSITION_INTEGRITY_FAILED",
            ";".join(f"{row.code}:{row.path}" for row in integrity.diagnostics),
        )
    envelope_id = (
        "CLASSICAL_RESOLUTION_FAILURE_EFFECT_DISPOSITION_ENVELOPE:"
        + object_sha256({
    ...
    return ClassicalResolutionFailureEffectDispositionEnvelope(
        resolution_failure_effect_envelope_id=envelope_id,
        source_final_effect_envelope_id=source_final_envelope.final_effect_envelope_id,
        source_final_effect_fact_hash=source_final_envelope.hashes.fact_hash,
        ...
        projected_resolution_failure_effect_disposition_ids=disposition_ids,
        ...
        algorithm_versions={
            "resolution_failure_effect_disposition": profile.algorithm_version
        },
        integrity=integrity,
        hashes=hashes,
    )
...
class BaziClassicalResolutionFailureEffectDispositionEngine:
    schema = "BAZI-CLASSICAL-RESOLUTION-FAILURE-EFFECT-DISPOSITION-RESULT-R1"
    typed_schema = (
        "BAZI-CLASSICAL-RESOLUTION-FAILURE-EFFECT-DISPOSITION-TYPED-RESOLUTION-R1"
    )
    ...
    def resolve_typed(
        ...
        try:
            profile = request.resolution_failure_effect_profile.validate()
            for status, code in (
                ...
                if status not in {"RESOLVED", "MULTI_CANDIDATE"}:
                    raise BaziClassicalResolutionFailureEffectDispositionError(code, status)
            ...
            return BaziClassicalResolutionFailureEffectDispositionResolution(
            ...
            code = getattr(
                exc,
                "diagnostic_code",
                "CLASSICAL_RESOLUTION_FAILURE_EFFECT_DISPOSITION_FAILED",
            )
            return BaziClassicalResolutionFailureEffectDispositionResolution(
                self.typed_schema,
                "FAILED",
    ...
    def resolve(
        ...
        return {
            ...
            "status": typed.status,
            "resolution_failure_effect_profile": json_value(
                request.resolution_failure_effect_profile
            ),
