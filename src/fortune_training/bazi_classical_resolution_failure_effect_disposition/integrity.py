...
from .profile import (
    ...
    FAILURE_CLOSURE_REQUIREMENT_ID,
...
def _failure_closure_row(candidate: Any) -> Any:
    matches = tuple(
        row
        for row in candidate.closure_governance_rows
        if row.closure_requirement_id == FAILURE_CLOSURE_REQUIREMENT_ID
    )
    if len(matches) != 1:
        raise ValueError(
            f"RESOLUTION_FAILURE_CLOSURE_ROW_CARDINALITY_MISMATCH:{candidate.final_candidate_id}:{len(matches)}"
        )
    row = matches[0]
    if row.runtime_dependency_status != EXPECTED_UPSTREAM_CLOSURE_STATUS:
        raise ValueError(
            f"RESOLUTION_FAILURE_CLOSURE_UPSTREAM_STATUS_MISMATCH:{candidate.final_candidate_id}:{row.runtime_dependency_status}"
        )
    if FAILURE_CLOSURE_REQUIREMENT_ID not in candidate.unresolved_classical_semantic_requirements:
        raise ValueError(
            f"RESOLUTION_FAILURE_CLOSURE_REQUIREMENT_NOT_DECLARED:{candidate.final_candidate_id}"
        )
    return row


def candidate_is_handled_resolution_failure(candidate: Any) -> bool:
    ...
    if candidate.mechanism_proposal_kind != HANDLED_MECHANISM_PROPOSAL_KIND:
        raise ValueError(
            f"RESOLUTION_FAILURE_MECHANISM_PROPOSAL_KIND_MISMATCH:{candidate.final_candidate_id}"
        )
    if candidate.source_claim_edge_class != HANDLED_SOURCE_CLAIM_EDGE_CLASS:
        raise ValueError(
            f"RESOLUTION_FAILURE_SOURCE_CLAIM_EDGE_CLASS_MISMATCH:{candidate.final_candidate_id}"
        )
    if candidate.effect_facet != HANDLED_EFFECT_FACET:
        raise ValueError(
            f"RESOLUTION_FAILURE_EFFECT_FACET_MISMATCH:{candidate.final_candidate_id}"
        )
    _failure_closure_row(candidate)
    return True


def resolution_failure_effect_disposition_id(candidate: Any) -> str:
    return "CLASSICAL_RESOLUTION_FAILURE_EFFECT_DISPOSITION:" + object_sha256({
        ...
        "resolution_mechanism_disposition": RESOLUTION_MECHANISM_DISPOSITION,
    })
...
def expected_candidate_projection(
    ...
    handled = candidate_is_handled_resolution_failure(candidate)
    closure_rows: tuple[ResolutionFailureClosureResolutionRow, ...] = ()
    dispositions: tuple[CandidateLocalResolutionFailureEffectDisposition, ...] = ()
    if handled:
        _failure_closure_row(candidate)
        closure_rows = (
            ResolutionFailureClosureResolutionRow(
                closure_requirement_id=FAILURE_CLOSURE_REQUIREMENT_ID,
                upstream_runtime_dependency_status=EXPECTED_UPSTREAM_CLOSURE_STATUS,
                unit9_local_closure_result=LOCAL_CLOSURE_RESULT,
                semantic_scope=DISPOSITION_SEMANTIC_SCOPE,
            ),
        )
        disposition_id = resolution_failure_effect_disposition_id(candidate)
        dispositions = (
            CandidateLocalResolutionFailureEffectDisposition(
                resolution_failure_effect_disposition_id=disposition_id,
                source_final_candidate_id=candidate.final_candidate_id,
                ...
                resolution_mechanism_disposition=profile.resolution_mechanism_disposition,
        ...
        status = "RESOLUTION_FAILURE_EFFECT_DISPOSITION_PROJECTED"
    else:
        status = "PRESERVED_NON_RESOLUTION_FAILURE_CANDIDATE"
    if status not in CANDIDATE_PROJECTION_STATUSES:
        raise ValueError(f"UNKNOWN_UNIT9_CANDIDATE_PROJECTION_STATUS:{status}")
    disposition_ids = tuple(
        row.resolution_failure_effect_disposition_id for row in dispositions
    )
    projection_id = (
        "CLASSICAL_RESOLUTION_FAILURE_EFFECT_CANDIDATE_PROJECTION:"
        + object_sha256({
            "source_final_candidate_id": candidate.final_candidate_id,
            "projection_status": status,
            "resolution_failure_closure_rows": json_value(closure_rows),
            "resolution_failure_effect_disposition_ids": disposition_ids,
        })
    )
    return ResolutionFailureEffectCandidateProjection(
        candidate_projection_id=projection_id,
        source_final_candidate_id=candidate.final_candidate_id,
        source_final_candidate=candidate,
        projection_status=status,
        resolution_failure_closure_rows=closure_rows,
        resolution_failure_effect_dispositions=dispositions,
        resolution_failure_effect_disposition_ids=disposition_ids,
    )
...
def expected_fragment_projection(
    ...
    disposition_ids = tuple(
        ...
        for disposition_id in candidate.resolution_failure_effect_disposition_ids
    )
    if not fragment.final_candidates:
        status = "PRESERVED_ZERO_CANDIDATES"
    elif disposition_ids:
        status = "RESOLUTION_FAILURE_EFFECT_DISPOSITIONS_PROJECTED"
    else:
        status = "PRESERVED_NO_RESOLUTION_FAILURE_EFFECT_DISPOSITIONS"
    projection_id = (
        "CLASSICAL_RESOLUTION_FAILURE_EFFECT_FRAGMENT_PROJECTION:"
        + object_sha256({
    ...
    return ResolutionFailureEffectFragmentProjection(
...
def build_expected_source_record_candidate_sets(
...
) -> tuple[SourceRecordResolutionFailureEffectCandidateSet, ...]:
    ...
    if len(by_final_fragment_id) != len(fragment_projections):
        raise ValueError("DUPLICATE_UNIT9_SOURCE_FINAL_FRAGMENT_ID")
    rows: list[SourceRecordResolutionFailureEffectCandidateSet] = []
    for source_set in source_final_envelope.source_record_candidate_sets:
        try:
            ...
            raise ValueError(
                f"UNIT9_SOURCE_RECORD_FRAGMENT_LINEAGE_MISSING:{exc.args[0]}"
            ) from exc
        candidate_projection_ids = tuple(
            candidate_id
        ...
        disposition_ids = tuple(
            disposition_id
            for fragment in selected
            for disposition_id in fragment.resolution_failure_effect_disposition_ids
        )
        set_id = (
            "CLASSICAL_RESOLUTION_FAILURE_EFFECT_SOURCE_RECORD_SET:"
            + object_sha256({
        ...
        rows.append(SourceRecordResolutionFailureEffectCandidateSet(
...
def build_expected_indexes(
    ...
    for projection in candidate_projections:
        for disposition in projection.resolution_failure_effect_dispositions:
            ...
            effect["dispositions"].append(
                disposition.resolution_failure_effect_disposition_id
            )
        ...
        for row in projection.resolution_failure_closure_rows:
            by_closure.setdefault(
                (row.closure_requirement_id, row.unit9_local_closure_result), []
    ...
    effect_index = tuple(
        EffectChannelResolutionFailureDispositionIndexEntry(
            ...
            resolution_failure_effect_disposition_ids=tuple(value["dispositions"]),
    ...
    closure_index = tuple(
        LocalFailureClosureResolutionIndexEntry(
            ...
            unit9_local_closure_result=key[1],
...
def resolution_failure_effect_hash_bundle(
    source_final_envelope: Any,
    ...
    return ResolutionFailureEffectDispositionHashBundle(
...
def validate_resolution_failure_effect_envelope(
    ...
    diagnostics: list[ResolutionFailureEffectDispositionIntegrityDiagnostic] = []
    ...
    if fragment_projections != expected_fragments:
        diagnostics.append(ResolutionFailureEffectDispositionIntegrityDiagnostic(
            "UNIT9_FRAGMENT_SEMANTIC_REPLAY_MISMATCH",
            "fragment_projections",
            "Unit 9 fragments do not replay exactly from Unit 7",
        ))
    ...
    if source_record_candidate_sets != expected_source_sets:
        diagnostics.append(ResolutionFailureEffectDispositionIntegrityDiagnostic(
            ...
            "Unit 9 source-record factorization differs from exact Unit 7 lineage replay",
        ))
    ...
    expected_disposition_ids = tuple(
        ...
        for disposition_id in row.resolution_failure_effect_disposition_ids
    )
    if projected_candidate_projection_ids != expected_projection_ids:
        diagnostics.append(ResolutionFailureEffectDispositionIntegrityDiagnostic(
            "UNIT9_CANDIDATE_PROJECTION_ID_REPLAY_MISMATCH",
            "projected_candidate_projection_ids",
            "candidate projection identity set/order drifted",
        ))
    if projected_resolution_failure_effect_disposition_ids != expected_disposition_ids:
        diagnostics.append(ResolutionFailureEffectDispositionIntegrityDiagnostic(
            "UNIT9_RESOLUTION_FAILURE_DISPOSITION_ID_REPLAY_MISMATCH",
            "projected_resolution_failure_effect_disposition_ids",
            "resolution-failure disposition identity set/order drifted",
        ))
    expected_hashes = resolution_failure_effect_hash_bundle(
        source_final_envelope,
        fragment_projections,
        source_record_candidate_sets,
        effect_channel_index,
    ...
    if hashes != expected_hashes:
        diagnostics.append(ResolutionFailureEffectDispositionIntegrityDiagnostic(
            "UNIT9_HASH_REPLAY_MISMATCH",
            "hashes",
            "Unit 9 FactHash/ComputationHash mismatch",
        ))
