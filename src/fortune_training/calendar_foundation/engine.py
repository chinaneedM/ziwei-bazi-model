...
class TimeCalendarFoundation:
    ...
    def _resolve_bazi_candidate(
        ...
        """Resolve only the shared physical facts required by Bazi charting.

        The combined resolve() API remains unchanged. This projection exists so
        Bazi chart generation does not depend on Ziwei-only Chinese-calendar
        mapping or Ziwei policy choices.
        """
        ...
        previous_jie, next_jie = self.solar_terms.adjacent_terms(candidate.utc_instant, jie_only=True)
        trace.add(
            "resolve_solar_term_boundaries",
            "FACT",
            {"birth_utc": candidate.utc_instant},
            {"previous_jie": previous_jie, "next_jie": next_jie},
            {"comparison_coordinate": "UTC_TO_UTC"},
        )
        bazi = self.bazi.resolve(
            candidate.utc_instant,
    ...
    @staticmethod
    def _bazi_classification_signature(branch: dict[str, Any]) -> tuple[str, str, str, str]:
        bazi = branch["bazi_time"]
        return (
            bazi["year_pillar"],
            bazi["month_pillar"],
    ...
    def resolve_bazi(
        self,
        birth: BirthInput,
        selection: BaziPolicySelection | None = None,
    ) -> dict[str, Any]:
        """Resolve Bazi time/calendar candidates without Ziwei calendar work."""

        selected = self.policy_registry.validate_bazi_selection(
            selection or self.policy_registry.default_bazi_selection()
        )
        ...
        for wall_time in sampled_wall_times:
            ...
            for candidate in candidates:
                if candidate is not None:
                    branches.append(self._resolve_bazi_candidate(sampled_birth, candidate, selected, civil))

        signatures = {self._bazi_classification_signature(branch) for branch in branches}
        if not branches:
        ...
        return {
            ...
            "metadata": {
                ...
                "projection": "BAZI_ONLY",
                "facts_and_policies_separated": True,
                "ziwei_calendar_evaluated": False,
