...
def fixed_build_adjusted_matrix(matrix: dict[str, Any], audit: dict[str, Any], generic_fix: dict[str, Any]) -> dict[str, Any]:
    ...
    for atom in matrix['rows']:
        ...
        if new_direction != atom['direction_status']:
            changed_keys.append((atom['track_id'], atom['atom_id']))
        rows.append(clone)
    return r14.with_hash({
        'schema': 'DEV-GROUP-002-R14-ADJUSTED-ATOM-DIRECTION-MATRIX-V3',
        ...
        'change_count_key_mode': 'TRACK_ID_PLUS_ATOM_ID',
        'r14_v1_atom_id_only_collision_corrected': True,
        'r14_v2_cross_track_change_count_collision_corrected': True,
        'rows': rows,
        'row_count': len(rows),
        'changed_atom_keys': [{'track_id': track, 'atom_id': atom_id} for track, atom_id in changed_keys],
        'changed_atom_ids': [atom_id for _, atom_id in changed_keys],
        'changed_atom_count': len(changed_keys),
        'ziwei_changed_atom_count': sum(track == 'ZIWEI' for track, _ in changed_keys),
        'bazi_changed_atom_count': sum(track == 'BAZI' for track, _ in changed_keys),
