...
def main() -> int:
    spec = importlib.util.spec_from_file_location('r12_materializer', MODULE_PATH)
    if spec is None or spec.loader is None:
        raise SystemExit('cannot load R12 materializer')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    result: dict[str, object] = {'schema': 'DEV-GROUP-002-R12-PREFLIGHT-DIAGNOSTIC-V1'}
    try:
    ...
    try:
        input_freeze, *_ = module.build_input_freeze(ROOT)
    ...
    try:
        ...
        result['source_excerpt_count'] = excerpts.get('row_count')
