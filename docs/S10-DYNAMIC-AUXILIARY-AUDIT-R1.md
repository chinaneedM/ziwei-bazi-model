...
## Preserved unresolved candidates: 流天魁／流天钺
...
No 流魁／流钺 fact is emitted in R1. A later contract must expose the strict S01
table and the compatibility case method as named, independently hashed
candidates with separate source chains. Until that candidate-preserving schema
exists, selecting either table would violate the fail-closed rule.
