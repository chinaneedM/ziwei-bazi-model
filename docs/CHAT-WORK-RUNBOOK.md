...
同一案例连续3轮达到正确率门槛后才推进。第一轮是`FIRST_BLIND`；后续轮是`SAME_CASE_REPLAY`，只计入R1案例门槛，不计作新的首次盲测或跨案例验证。
...
GitHub会自动复核答案、记录题级统计和连续次数。未达到连续3轮时继续当前案例；达到第3次连续通过后才进入下一案例。
...
- 相关规则在未来不同案例中的支持与反证；
- 失败后候选规则的安全检查；
- 当前案例连续次数、是否达到推进门槛以及安全启动包刷新。

用户不需要人工分类题目、不需要判断哪些题能验证哪些规则，也不需要运行Git、gh、Python或终端命令。

## 五、重要边界

- 通过：不生成新规则；连续次数加1，未满3次仍留在当前案例。
- 失败：连续次数归零，只在`model-learning/`生成候选通用规则，然后继续当前案例。
- 不相关的后续题目不会验证候选规则。
- 同案后续轮计入连续3轮门槛，但不计首次盲测统计或跨案例规则验证。
