...
该通道不消耗ChatGPT Work额度。Chat负责当前案例各轮预测、揭盲复盘和生成提交单；GitHub Actions负责冻结、用加密答案复核、更新题级统计与连续次数、写入候选规则，并只在R1门槛达到后切换案例。
...
- 只接受仓库所有者创建的训练Issue；
- 确认当前案例仍未达到连续3轮门槛，并正确区分首次盲测与同案后续轮；
- 验证每题标签均来自固定taxonomy；
...
- 运行完整验证与测试；
- 刷新当前案例连续次数；仅在连续3轮通过后切换下一案例并更新 `chat-input/current.json`；
- 留下不含答案的结果并关闭Issue。

## 出错处理

- Issue成功并自动关闭：该轮闭环完成；下一次按安全包继续当前案例或在门槛达成后开始下一案例。
