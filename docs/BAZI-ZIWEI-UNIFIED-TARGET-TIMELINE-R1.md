...
## Bazi neutral structural-support projection

Each target-flow candidate separately composes the released Structural Support
Foundation. The fixed Natal month-command reference and active Flow solar-month
reference remain distinct and retain their parent occurrence/frame, interval,
rule, source, candidate lineage, and hashes. Exact hidden-stem matches and
same-element/different-stem support remain separate evidence candidates with
their affinity and exposure IDs; the month-command and active-flow-month scoped
candidate sets are never merged.

The workbench presents this evidence read-only and explicitly makes no root,
strength, weight, score, or 得令 conclusion. Application integrity validates the
projection independently, while full replay reconstructs the exact Support
Foundation from Natal, Flow, and Structural inputs. See
...
The projection preserves the existing civil-year annual selector behavior and adds the regular lunar-month frame identity, Ganzhi, and active address. It also carries immutable Daxian, Annual, and regular-Month layer projections instead of reducing those layers to selector IDs. Every layer projection records its own source layer, source stem, frame and parent-frame identity, temporal rule-set and algorithm identity, stable frame sources, four transformation activations, three independent 禄存／擎羊／陀罗 activations, two independent 流文昌／流文曲 activations, and a non-selected 流魁／流钺 method-candidate set. Daxian and Annual additionally preserve separate branch-basis 流天马 case-method candidates; the former uses the Daxian life-palace branch and the latter the annual branch. They are not promoted to facts and are not extended into Month, Day, or Hour. The strict S01 table and the Wenmo compatibility method keep separate method IDs, activation IDs, source chains, fact hashes, and computation hashes even when their non-辛 placements coincide. Equal-named transformations or auxiliaries in different layers retain different activation IDs and are never merged. The integrity path reconstructs every layer projection from the released source frame and compares every field, so recomputing local hashes cannot conceal a changed parent, stem or branch basis, rule identity, source chain, transformation, auxiliary, or candidate method.

The same candidate now carries a separate Minor-Limit ring encounter projection. S05 names 博士十二神、将前十二神、岁前十二神 as the three rings used when a Minor Limit encounters them, but supplies no rule for regenerating those rings from the Minor-Limit palace. The runtime therefore reads the one existing natal-ring member at the selected Minor-Limit address and preserves the source ring's anchor, direction, generator/version, sources and member identity. It is explicitly marked `SOURCE_DIRECTED_NATAL_RING_ENCOUNTER_NO_REGENERATION`, independently hashed and fully replayed; it is not a new moving-ring placement and contains no interpretation.
...
The application-flow replay validates all seven Bazi layers, Xiaoyun candidate preservation, exact equality with the upstream Dayun/Flow/Daily-Hourly facts, the complete temporal classical annotation projection, the neutral Dayun/Annual/Monthly Structural projection, and the separate Structural Support projection with their independent source and projection hashes. The shared Ziwei projection replay independently reconstructs its selected Minor-Limit frame and three natal-ring encounters, its Daxian/Annual/regular-Month layer projections and their hashes from source frames, then reconstructs its effective lunar date, daily frame, daily twelve-designation and 禄羊陀／流昌曲 facts, and both time-standard hour candidates with their case-scoped address/designation/禄羊陀／流昌曲 facts (or the unresolved leap-month parent state).
