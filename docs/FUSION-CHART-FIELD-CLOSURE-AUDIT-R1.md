...
This audit freezes the current deterministic Ziwei + Bazi fusion charting baseline and classifies remaining work by field family. It is a product/charting audit only. It does not authorize prediction, interpretation, model training, winner selection among disputed schools, or silent reconciliation of Ziwei and Bazi time conventions.
...
The R2 combined target-flow fusion now binds the exact same target-coordinate resolution into the released Bazi target-flow bundle and Ziwei selector projection while preserving independent subsystem semantics.
...
## F. Ziwei structural relations — CLOSED FOR RELEASED R1-R5
...
The current Sanhe interaction controller and browser sidecar consume typed structure identity rather than SVG geometry.

Items such as Qishu position, Yiliu Gongzong, Jia Gong, pair-geometry strength or motif/configuration compilers remain separate semantic/product backlog unless a concrete deterministic chart defect is demonstrated.

Status: **CLOSED for released R1-R5; semantic extensions OPEN**.
...
The fusion R2 target-flow layer now exposes Ziwei selector candidates from the same explicit target coordinate used by Bazi flow.
...
The combined application now has these independent, hash-bound layers:
...
This is the next active development class. It should be handled as a field-by-field audit against the actual workbench, Wenmo reference screenshots and Wenzhen reference screenshots.
...
2. Build a machine-readable/display-oriented field parity matrix.
3. Mark every current workbench field as visible / hidden / missing.
4. Close ALREADY_RELEASED_NOT_YET_VISIBLE items first.
5. Implement deterministic missing fields one family at a time with source/profile tests.
...
At commit `cf6a4fa952d242384f0dce6083cb5a13802a1af0`, the mandatory deterministic fusion core is closed through Combined Target-Flow Fusion R2 and its workbench/CI integration.

The active development frontier is now **field parity and product closure**, not another rewrite of the time/calendar, Bazi natal/flow, Ziwei natal/structural, or fusion identity foundations.
