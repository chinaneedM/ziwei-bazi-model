...
- **Batch 12AJ (Fullbook 羅經 timekeeping semantics):** Nanyangtang and Guangyi/Yulgok physical target pages directly agree on `如天氣陰雨之際必須羅經以定真確時候若差訛則命不凖矣`. Contemporary Ming technical controls separate compass direction from timekeeping—日晷 day, 星晷 night, 正線羅經 meridian direction, 行漏 inclement-weather supplement—and warn that exclusive compass use can create time error. Ming 《羅經頂門針》 independently anchors 羅經 in magnetic-needle/directional semantics. Thus the Fullbook sentence does **not** directly mean “use true/apparent solar time”; its exact operational bridge remains unresolved. `HPA-ZDATE-006` stays `MISSING_FROM_PRODUCT`; no algorithm reopen. See `docs/FUSION-CHART-HISTORICAL-PROVENANCE-AUDIT-BATCH-12-ZIWEI-FULLBOOK-LUOJING-TIMEKEEPING-SEMANTICS-AJ.md`.
