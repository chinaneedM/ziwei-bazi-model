...
截至 Batch 11P，Matrix 仍为 197 个 rule/field families、165 行完成实审；HISTORICALLY_SUPPORTED=87，identified missing candidate families=13，chart algorithm defect=0，algorithm reopen=0，provenance defect=9/9 repaired。11B/11C 把大运真实交运历日与现代日历实现拆开并建立 fail-closed historical-calendar adapter contract；11D-11G 闭合明大统历一手方法源、1578 官方实录月朔 oracle、D1/迟疾行度生产算法与子正/100刻内部时间坐标；11H/11I 完成 1578 的 13/13 日级源算法重放与同年 NCL 06313 钦天监原历 12/12 月页直接影像校勘；11J 闭合 1569 主表的阶段化精度图并否定“全局统一舍入法”。11K/11L 完成 1673 小川本 NDL/九州独立馆藏校勘并建立 L124 捷法/倒数层证据边界；11M 保持 G893 现存本精确印年在世宗 1418–1450 区间内未决，并绑定公开实物开篇图而保持六个 target 全部 fail-closed。11N 建立规章阁 G894 与《世宗实录》两条独立早期朝鲜比较路线，11O 又从国史编纂委员会太白山史库官方原图直接闭合 Sillok 五个太阴控制点：L8=10.5601775、L101=5.20481125 且保留显式位置零、L114=9日3489、L124=1.0281、L132=7.886075 且保留显式零，太阳 D16 继续待官方续页图。11P 则通过规章阁 live renderer 直接绑定 `七政算內篇 奎貴894-v.1-3`（`BOOK_CD=GK00894_00` / `ITEM_CD=GJB`），并用 G894 自身 `018a/041b` 方法正文闭合 `限數 / 遲疾曆日率 / 損益分 / 遲疾度 / 疾曆限行度 / 遲曆限行度` 六列机械语义；随后无 OCR 原图校勘闭合五个太阴控制点：L8=10.5601775，L101=5.20481125（`五度二十〇四八一一二五`），L114=9日3489，L124=1.0281（`疾一度〇二八一`），L132=7.886075（`損七分八八六〇七五`）。G894 太阳十六日页面虽已直接绑定，但该表只打印 `積日 / 盈縮加分 / 盈縮積`，缺失当前 D16 control 所针对的第二差分字段，因此判为结构不可比而不是填零/空值/邻列值。G894 的 L8 与高丽史/Sillok 支一致而 L124 与明1569/Sillok 支一致，再次证明必须逐单元格、逐机械字段分析，禁止整本“站队”与来源多数票。G894、Sillok、G893 仍是不同文献/实物层；G893 六个目标仍全部 `PENDING_DIRECT_TARGET_PAGE`。历史历法 runtime 继续 fail-closed。最新批次见 `docs/FUSION-CHART-HISTORICAL-PROVENANCE-AUDIT-BATCH-11-BAZI-JOSEON-G894-NATIVE-COLLATION-P.md`。
