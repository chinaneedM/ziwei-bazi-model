...
截至 Batch 10C，Matrix 已扩展到 187 个 rule/field families，其中 154 行完成实审；HISTORICALLY_SUPPORTED=84，当前 `MISSING_FROM_PRODUCT` 行降回 8，累计 identified missing candidate families 仍保留为 12；chart algorithm defect=0，algorithm reopen=0。10C 新增第二套 source-scoped historical candidate registry/runtime：`BAZI-HISTORICAL-RELATION-CANDIDATES-R1`，固定 `PRESERVED_NOT_SELECTED`，可独立输出辰戌丑未四土局、四方/三会、宋系早期四破、同柱干藏五合；released raw core 与默认 fact-hash 语义不变。与此同时 `PROV-DEFECT-008` 修复了《命理探源》卷五关系页误借夜子时 source ID 的范围错误。Batch 10C 见 `docs/FUSION-CHART-HISTORICAL-PROVENANCE-AUDIT-BATCH-10-BAZI-HISTORICAL-RELATION-CANDIDATES-C.md`。
