...
截至 Batch 11A，Matrix 已扩展到 190 个 rule/field families，其中 158 行完成实审；HISTORICALLY_SUPPORTED=86，chart algorithm defect=0，algorithm reopen=0，provenance defect=9/9 repaired。11A 将藏干“成员集合”“古籍文本原序”“当前软件稳定显示序”“后世本气/余气层级”彻底拆开：成员仍按《渊海子平》闭合；当前 `registry_ordinal` 只保留 compatibility/lineage，不等同本气、中气、余气，也不代表强弱。动态 Dayun/Xiaoyun/年/月/日/时藏干确认只是复用同一成员表。`PROV-DEFECT-009` 修复了动态 annotation 曾把显示顺序纳入 FactHash 的语义不一致：现在成员决定 FactHash，顺序只进入 ComputationHash；输出顺序和全部排盘坐标不变。Batch 11A 见 `docs/FUSION-CHART-HISTORICAL-PROVENANCE-AUDIT-BATCH-11-BAZI-HIDDEN-STEM-ORDER-A.md`。
