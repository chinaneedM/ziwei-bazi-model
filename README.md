...
截至 Batch 11B，Matrix 已扩展到 197 个 rule/field families，其中 165 行完成实审；HISTORICALLY_SUPPORTED=87，identified missing candidate families=13，chart algorithm defect=0，algorithm reopen=0，provenance defect=9/9 repaired。11A 将藏干成员、古籍文本原序、当前稳定显示序与后世本气/余气层级拆开，保持显示顺序仅作为 lineage；11B 进一步拆解大运“节距测量 → 三日一年象征年龄 → 真实交运历日 → 后续十年换运边界”，确认宋明文献支持离散日/时辰折算，而《三命通会》的小月、闰月校正属于历史阴阳历日历化规则族。当前 UTC×120、问真 Gregorian month displacement 与 Gregorian 十年周年均继续明确标注为现代实现；历史阴阳历交运候选保持 MISSING_FROM_PRODUCT，必须先建立 source/regime-scoped historical-calendar adapter，严禁直接复用现代 `ChineseCalendarEngine` 充当古历权威。Batch 11A/11B 分别见 `docs/FUSION-CHART-HISTORICAL-PROVENANCE-AUDIT-BATCH-11-BAZI-HIDDEN-STEM-ORDER-A.md` 与 `docs/FUSION-CHART-HISTORICAL-PROVENANCE-AUDIT-BATCH-11-BAZI-DAYUN-CALENDAR-REALIZATION-B.md`。
