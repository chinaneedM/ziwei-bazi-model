...
截至 Batch 11M，Matrix 仍为 197 个 rule/field families、165 行完成实审；HISTORICALLY_SUPPORTED=87，identified missing candidate families=13，chart algorithm defect=0，algorithm reopen=0，provenance defect=9/9 repaired。11B/11C 把大运真实交运历日与现代日历实现拆开并建立 fail-closed historical-calendar adapter contract；11D-11G 闭合明大统历一手方法源、1578 官方实录月朔 oracle、D1/迟疾行度生产算法与子正/100刻内部时间坐标；11H/11I 完成 1578 的 13/13 日级源算法重放与同年 NCL 06313 钦天监原历 12/12 月页直接影像校勘；11J 闭合 1569 主表的阶段化精度图并否定“全局统一舍入法”。11K/11L 把 1673 小川正意《授時暦經立成》的 NDL 与九州大学两个独立馆藏分开做无 OCR 原图校勘：D16 均因表结构缺少独立日差/消息分型字段而不可直接比较，L114 均直接为 `九日三四八九`，九州 L124 的 `0.0797587/0.0704164` 被严格限定为支持明本 raw `1.0281/1.1645` 谱系的捷法/倒数层证据而非 raw 字面。11M 则修正 G893 早期实物的年代与访问前提：规章阁 provider 仅把现存甲寅字本定在世宗 1418–1450，KOSTMA 与李亮 2018 指向 1434，而李亮后续数表研究把同一 collection no. 893 记作 1444，因此现存本精确印年保持未决，禁止用 1434/1444 作为数字异文的时间权重捷径；李亮 2023 Figure 1 已作为公开的实物二次复制图绑定，可直接见 `授時曆立成卷上`、王恂奉敕撰、`太陽冬至前後二象盈初縮末限` 与初日至八日，但它不是 D16 或任何太阴目标页，六个 G893 control 仍全部 `PENDING_DIRECT_TARGET_PAGE`。三个 Dayun calendarized candidate 继续 `MISSING_FROM_PRODUCT`，历史历法 runtime 仍保持 fail-closed。最新批次见 `docs/FUSION-CHART-HISTORICAL-PROVENANCE-AUDIT-BATCH-11-BAZI-G893-PROVENANCE-PUBLIC-FIGURE-M.md`。
