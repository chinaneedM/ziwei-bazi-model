...
截至 Batch 11J，Matrix 仍为 197 个 rule/field families、165 行完成实审；HISTORICALLY_SUPPORTED=87，identified missing candidate families=13，chart algorithm defect=0，algorithm reopen=0，provenance defect=9/9 repaired。11B/11C 把大运真实交运历日与现代日历实现拆开并建立 fail-closed historical-calendar adapter contract；11D-11G 闭合明大统历一手方法源、1578 官方实录月朔 oracle、D1/迟疾行度生产算法与子正/100刻内部时间坐标；11H/11I 完成 1578 的 13/13 日级源算法重放与同年 NCL 06313 钦天监原历 12/12 月页直接影像校勘。1569 周相本的太阳 185/185、太阴迟疾 169/169、太阴行度 169/169 原刻逐行校勘已经闭合。11J 进一步把精度拆成阶段规则：日率列 168/168 向下取整，损益捷法 168/168 截尾，迟/疾行度除两个中央原刻特例外 334/334 向上进位，行度捷法 336/336 截尾；因此原刻证据直接否定了“全局统一舍入法”。这只闭合 1569 主表生成精度图，不把 1596 单一大统算例的动态截位深度推广为所有年月；动态插值/定朔精度、跨版本异文原图成因、朔经度、多年闰月、十年递推与历史历法 runtime 仍保持未闭合。三个 Dayun calendarized candidate 继续 `MISSING_FROM_PRODUCT`。最新批次见 `docs/FUSION-CHART-HISTORICAL-PROVENANCE-AUDIT-BATCH-11-BAZI-MING-DATONG-FIXED-POINT-PRECISION-J.md`。
