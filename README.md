...
截至 Batch 11O，Matrix 仍为 197 个 rule/field families、165 行完成实审；HISTORICALLY_SUPPORTED=87，identified missing candidate families=13，chart algorithm defect=0，algorithm reopen=0，provenance defect=9/9 repaired。11B/11C 把大运真实交运历日与现代日历实现拆开并建立 fail-closed historical-calendar adapter contract；11D-11G 闭合明大统历一手方法源、1578 官方实录月朔 oracle、D1/迟疾行度生产算法与子正/100刻内部时间坐标；11H/11I 完成 1578 的 13/13 日级源算法重放与同年 NCL 06313 钦天监原历 12/12 月页直接影像校勘；11J 闭合 1569 主表的阶段化精度图并否定“全局统一舍入法”。11K/11L 完成 1673 小川本 NDL/九州独立馆藏校勘并建立 L124 捷法/倒数层证据边界；11M 修正 G893 现存本精确印年为世宗 1418–1450 区间内未决，并绑定公开实物开篇图而保持六个 target 全部 fail-closed。11N 新增两条**独立早期朝鲜比较路线**：规章阁直接著录 `七政算內篇 奎貴894-v.1-3` 为李純之、金淡受命編、甲寅字、1444，并提供原文图像/原文文本服务；国史编纂委员会《世宗实录》卷156又把 `太陽冬至前後二象盈初縮末限` 精确定位到太白山史库本 `60冊 156卷 6張 A面`（wda_50016011），把 `太陰限數遲疾度` 定位到 `60冊 156卷 13張 A面`（wda_50016016）。G894、世宗实录与 G893 必须保持为不同文献/实物层，馆藏号 893/894 相邻不得推成同本谱系；目前 Sillok 已直接闭合五个太阴控制点，太阳 D16 仍待直接续页图；G894/G893 六个控制值仍全部待直接原图单元格判读，禁止从明本、高丽史或小川本预填。三个 Dayun calendarized candidate 继续 `MISSING_FROM_PRODUCT`，历史历法 runtime 仍保持 fail-closed。11O 又从国史编纂委员会太白山史库官方原图直接闭合五个太阴控制点：L8=10.5601775，L101=5.20481125 且保留显式位置零，L114=9日3489，L124=1.0281，L132=7.886075 且保留显式零。L8 与《高丽史》接收支一致而 L124 与明1569/小川机械证据同侧，故证据必须逐单元格裁决，禁止整本“站队”或来源多数票。太阳 D16 因 6A 原图不含目标且后续官方图树/运输当次不可达而继续 fail-closed。最新批次见 `docs/FUSION-CHART-HISTORICAL-PROVENANCE-AUDIT-BATCH-11-BAZI-JOSEON-SILLOK-NATIVE-COLLATION-O.md`。
