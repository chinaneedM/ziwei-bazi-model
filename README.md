...
截至 Batch 08A，Matrix 已扩展到 153 个 rule/field families，其中 107 行完成实审；已确认并在 provenance 层 forward-only 修复/隔离 7 个 metadata defects，仍未发现 chart algorithm defect，也没有任何 algorithm reopen。Batch 08A 开始审计紫微动态辅助层：received《紫微斗数全书》直接见证流年禄存/擎羊/陀罗；大限与更细层流曜、流昌流曲、流魁钺、运马/流马则按王亭之《安星法及推断实例》明确降为中州派 school-scoped method，而不是古法通则。Wenmo 流魁钺继续只作 compatibility candidate。代码中把 S01 严格流魁钺候选错误标成 `CANONICAL_SOURCE_TABLE` 的来源语义缺陷已作为 PROV-DEFECT-007 修复为 `S01_STRICT_PROJECT_CORPUS_METHOD`，坐标、候选集合和选择政策均未改变。Batch 08A 见 `docs/FUSION-CHART-HISTORICAL-PROVENANCE-AUDIT-BATCH-08-ZIWEI-DYNAMIC-AUXILIARIES-A.md`。
