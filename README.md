...
截至 Batch 11C，Matrix 仍为 197 个 rule/field families、165 行完成实审；HISTORICALLY_SUPPORTED=87，identified missing candidate families=13，chart algorithm defect=0，algorithm reopen=0，provenance defect=9/9 repaired。11B 已将大运“节距测量 → 三日一年象征年龄 → 真实交运历日 → 后续十年换运边界”拆开，确认历史阴阳历日历化是独立缺口；11C 进一步建立 `HISTORICAL-CHINESE-CALENDAR-ADAPTER-CONTRACT-R1`，把明代 `大统历` 语境与 1645 后 `时宪历` 语境分离，并在代码层强制 fail-closed：未有 source/regime-scoped 历算实现前，不得以现代 `ChineseCalendarEngine`、Gregorian 周年或 UTC 持续时间替代古历。三个 Dayun calendarized candidate 仍保持 `MISSING_FROM_PRODUCT`，没有算法重开。Batch 11B/11C 分别见 `docs/FUSION-CHART-HISTORICAL-PROVENANCE-AUDIT-BATCH-11-BAZI-DAYUN-CALENDAR-REALIZATION-B.md` 与 `docs/FUSION-CHART-HISTORICAL-PROVENANCE-AUDIT-BATCH-11-BAZI-HISTORICAL-CALENDAR-ADAPTER-C.md`。
