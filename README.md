...
- 同一案例必须连续3轮达到本轮正确率门槛才进入下一案例；任一轮未达标，连续次数归零。
- 少于 5 题必须全对；5题及以上达到向上取整的80%记为该轮通过。该阈值只描述本轮表现，不代表整个模型成熟。
- 通过但尚未达到连续3轮时仍留在同一案例；达到第3次连续通过后才推进。
- 未通过时只允许在`model-learning/`形成不含答案映射的通用修正，然后回到同一案例重新累计。
- 第一轮保留 `FIRST_BLIND` 标签用于首次盲测统计；后续同案轮计入R1案例门槛，但永不冒充跨案例规则验证证据。
...
6. 未通过时自动校验并加入候选规则、清零连续次数并留在原案例；通过轮累计连续次数，仅在达到3次时进入下一案例。
...
验证覆盖冻结原典、答案隔离、模型发布链、题级标签、规则证据账本、R1同案连续3轮状态机、安全 Chat 输入包以及 Issue 自动闭环。
