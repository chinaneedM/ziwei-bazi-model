...
截至 Batch 11H，Matrix 仍为 197 个 rule/field families、165 行完成实审；HISTORICALLY_SUPPORTED=87，identified missing candidate families=13，chart algorithm defect=0，algorithm reopen=0，provenance defect=9/9 repaired。11B/11C 把大运真实交运历日与现代日历实现拆开并建立 fail-closed historical-calendar adapter contract；11D-11G 又闭合明大统历一手方法源、1578 官方实录月朔 oracle、D1/迟疾行度生产算法与子正/100刻内部时间坐标。11H 进一步用 1569 方法及重构数表从源头完整重放 1578：万历六年十二个月加次年正月锚点共 13/13 日级吻合，且同年 NCL 06313 钦天监原历已有 11/12 月页直接影像校勘、0 个大小月冲突；六月页因当前 PDF 渲染器单页故障仍保留为未直接认证，不从前后页或 oracle 推断。北京大学善本书目另著录同年一部 `528.7/1578`，目前只作第二实物谱系的书目证据。三个 Dayun calendarized candidate 仍保持 `MISSING_FROM_PRODUCT`；逐行原刻校勘、通用进位/截位、朔经度、多年闰月与十年递推未闭合前，禁止启用历史历法 runtime。最新批次见 `docs/FUSION-CHART-HISTORICAL-PROVENANCE-AUDIT-BATCH-11-BAZI-MING-DATONG-1578-D1-SOURCE-REPLAY-H.md`。
