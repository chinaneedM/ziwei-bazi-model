...
截至 Batch 11Q，Matrix 仍为 197 个 rule/field families、165 行完成实审；HISTORICALLY_SUPPORTED=87，identified missing candidate families=13，chart algorithm defect=0，algorithm reopen=0，provenance defect=9/9 repaired。11B/11C 把大运真实交运历日与现代日历实现拆开并建立 fail-closed historical-calendar adapter contract；11D-11G 闭合明大统历一手方法源、1578 官方实录月朔 oracle、D1/迟疾行度生产算法与子正/100刻内部时间坐标；11H/11I 完成 1578 的 13/13 日级源算法重放与同年 NCL 06313 钦天监原历 12/12 月页直接影像校勘；11J 闭合 1569 主表的阶段化精度图并否定“全局统一舍入法”。11K/11L 完成 1673 小川本 NDL/九州独立馆藏校勘并建立 L124 捷法/倒数层证据边界；11M 保持 G893 现存本精确印年在世宗 1418–1450 区间内未决，并绑定公开实物开篇图而保持六个 target 全部 fail-closed。11N 建立规章阁 G894 与《世宗实录》两条独立早期朝鲜比较路线；11O 从国史编纂委员会太白山史库官方原图直接闭合 Sillok 五个太阴控制点；11P 通过规章阁 live renderer 直接绑定 `七政算內篇 奎貴894-v.1-3` 并用自身 `018a/041b` 方法正文闭合六列机械语义及五个太阴控制点，同时判定该本太阳 D16 目标字段结构缺失。11Q 则收口 G893 图像访问拓扑：GitHub-hosted Ubuntu、macOS、Windows 的只读 renderer 路线均在应用响应前被重置，因此等价 hosted-runner 重试正式禁止；另一件规章阁对象经非 GitHub 路径成功进入 Wikimedia Commons 只证明 renderer 并非全局不可达，不构成 G893 页证据。旧《奎章閣漢文文獻珍藏》DVD04 目录中的《授時曆立成》仅登记为未验证 PDF/DJVU 镜像 locator，实际文件尚未取得且未绑定 `GK00893_00`；Yu Gyung Ro 1997 又独立支持《授時曆立成》与姜保《授時曆捷法立成》为不同书，继续禁止互相代替。G893 六个目标仍全部 `PENDING_DIRECT_TARGET_PAGE`，未从高丽史、明本、小川本、G894、Sillok、姜保本或镜像目录预填任何数值。历史历法 runtime 继续 fail-closed。最新批次见 `docs/FUSION-CHART-HISTORICAL-PROVENANCE-AUDIT-BATCH-11-BAZI-G893-ACCESS-MIRROR-ROUTES-Q.md`。
