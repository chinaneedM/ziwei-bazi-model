...
class BaziClassicalEffectSemanticCandidateIntegrityR1Tests(unittest.TestCase):
    ...
    def test_duplicate_admission_outer_lineage_fails_closed(self):
        duplicate = replace(
            self.admission,
        ...
        self.assertEqual("FAILED", result.status)
        self.assertTrue(any(
            "UPSTREAM_ADMISSION_OUTER_LINEAGE_PROJECTED_MORE_THAN_ONCE" in row
            for row in result.diagnostics
        ))

    def test_claim_facet_mapping_drift_fails_closed(self):
        effect_outer = self.effect.candidates[0]
        ...
        with self.assertRaisesRegex(ValueError, "UNIT2_CLAIM_FACET_MAPPING_MISMATCH"):
            project_fragment_semantic_candidates(
                admission_outer, admitted, bad_fragment, self.profile
            )

    def test_candidate_preservation_profile_releases_no_solver_semantics(self):
        self.assertEqual("NOT_RELEASED", self.profile.candidate_truth_semantics)
        self.assertEqual("NOT_RELEASED", self.profile.candidate_coexistence_semantics)
        self.assertEqual("NOT_RELEASED", self.profile.candidate_exclusivity_semantics)
        self.assertEqual("NOT_RELEASED", self.profile.candidate_priority_semantics)
        self.assertEqual("NOT_RELEASED", self.profile.candidate_conflict_semantics)
        self.assertEqual("NOT_RELEASED", self.profile.candidate_rewrite_semantics)
        self.assertEqual("NOT_RELEASED", self.profile.candidate_state_transition_semantics)
        self.assertEqual("NOT_RELEASED", self.profile.candidate_winner_loser_semantics)
        self.assertEqual("NOT_RELEASED", self.profile.cross_outer_composition)
        self.assertEqual("NOT_RELEASED", self.profile.cartesian_expansion)
