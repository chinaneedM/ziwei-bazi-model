...
from fortune_training.bazi_classical_reversal_reappearance_effect_disposition.engine import (
    _project_envelope,
)
import test_bazi_classical_final_effect_candidate_envelope_r1 as unit7_tests
import test_bazi_classical_reversal_reappearance_effect_disposition_r1 as unit10_tests


class BaziClassicalReversalReappearanceEffectDispositionIntegrityR1Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        unit10_tests.BaziClassicalReversalReappearanceEffectDispositionR1Tests.setUpClass()
        cls.profile = bazi_classical_reversal_reappearance_effect_disposition_r1_profile()
        cls.real_request = (
            unit10_tests.BaziClassicalReversalReappearanceEffectDispositionR1Tests.cross[0]
        )
    ...
    def _synthetic_source_outer(cls):
        ...
        controlled_fragment = replace(
            ...
            source_semantic_candidate_ids=(reversal.source_semantic_candidate_id,),
            source_mechanism_proposal_ids=(reversal.source_mechanism_proposal_id,),
            source_allocation_elaboration_ids=(reversal.source_allocation_elaboration_id,),
            final_candidates=(reversal,),
            final_candidate_ids=(reversal.final_candidate_id,),
        )
    ...
    def _recomputed_hashes(self, *, fragments=None, source_sets=None, lineage=None):
        ...
        return reversal_reappearance_effect_hash_bundle(
            ...
            envelope.projected_reversal_reappearance_effect_disposition_ids,
            envelope.lineage_binding_keys if lineage is None else lineage,
            self.profile,
        )

    def _validate(self, *, fragments=None, source_sets=None, lineage=None, hashes=None):
        envelope = self.synthetic_envelope
        ...
        return validate_reversal_reappearance_effect_envelope(
            self.synthetic_source,
            fragments,
            source_sets,
            envelope.effect_channel_index,
    ...
    def test_recomputed_unit10_hash_cannot_hide_global_restoration_tamper(self):
        ...
        changed_disposition = replace(
            disposition,
            source_asserted_disposition="GLOBAL_RESTORED",
        )
        changed_projection = replace(
            projection,
            reversal_reappearance_effect_dispositions=(changed_disposition,),
        )
    ...
    def test_recomputed_unit10_hash_cannot_hide_source_record_factorization_tamper(self):
        envelope = self.synthetic_envelope
        changed = replace(
    ...
    def test_tampered_unit7_envelope_fails_full_replay_before_unit10_projection(self):
        request = self.real_request
        source = request.source_final_effect_resolution
        changed_outer = replace(source.candidates[0], candidate_truth_semantics="RELEASED")
        changed_source = replace(source, candidates=(changed_outer, *source.candidates[1:]))
    ...
    def test_changed_mapping_and_closure_status_fail_closed(self):
        ...
        bad_mechanism = replace(candidate, mechanism_proposal_kind="RESOLUTION_MECHANISM_PROPOSAL")
        with self.assertRaises(ValueError):
            unit10_tests.expected_candidate_projection(bad_mechanism, self.profile)
        ...
        with self.assertRaises(ValueError):
            unit10_tests.expected_candidate_projection(
                replace(candidate, closure_governance_rows=(bad_row,)), self.profile
            )

    def test_recomputed_unit10_hash_cannot_hide_lineage_binding_tamper(self):
        envelope = self.synthetic_envelope
        changed_lineage = (
            *envelope.lineage_binding_keys[:-1],
            "REVERSAL_REAPPEARANCE_EFFECT_DISPOSITION_PROFILE:TAMPERED:9.9.9",
        )
