...
from fortune_training.bazi_classical_resolution_effect_disposition.engine import (
    _project_envelope,
)
import test_bazi_classical_final_effect_candidate_envelope_r1 as unit7_tests
import test_bazi_classical_resolution_effect_disposition_r1 as unit8_tests


class BaziClassicalResolutionEffectDispositionIntegrityR1Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        unit8_tests.BaziClassicalResolutionEffectDispositionR1Tests.setUpClass()
        cls.profile = bazi_classical_resolution_effect_disposition_r1_profile()
        cls.real_request = unit8_tests.BaziClassicalResolutionEffectDispositionR1Tests.cross[0]
    ...
    def _synthetic_source_outer(cls):
        ...
        resolution = unit8_tests.BaziClassicalResolutionEffectDispositionR1Tests.resolution_candidate
        ...
        controlled_fragment = replace(
            ...
            source_semantic_candidate_ids=(resolution.source_semantic_candidate_id,),
            source_mechanism_proposal_ids=(resolution.source_mechanism_proposal_id,),
            source_allocation_elaboration_ids=(resolution.source_allocation_elaboration_id,),
            final_candidates=(resolution,),
            final_candidate_ids=(resolution.final_candidate_id,),
        )
        fragments = (controlled_fragment, *real_outer.fragment_envelopes[1:])
        return replace(real_outer, fragment_envelopes=fragments)

    def _recomputed_hashes(self, *, fragments=None, source_sets=None):
        envelope = self.synthetic_envelope
        return resolution_effect_hash_bundle(
    ...
    def _validate(self, *, fragments=None, source_sets=None, hashes=None):
        envelope = self.synthetic_envelope
        fragments = envelope.fragment_projections if fragments is None else fragments
        source_sets = envelope.source_record_candidate_sets if source_sets is None else source_sets
        hashes = envelope.hashes if hashes is None else hashes
        return validate_resolution_effect_envelope(
            self.synthetic_source,
            fragments,
            source_sets,
            envelope.effect_channel_index,
    ...
    def test_recomputed_unit8_hash_cannot_hide_raw_relation_mutation_semantics(self):
        envelope = self.synthetic_envelope
        ...
        changed_disposition = replace(disposition, raw_relation_action="DELETE")
        changed_projection = replace(
            projection,
            resolution_effect_dispositions=(changed_disposition,),
        )
        changed_fragment = replace(
            fragment,
            candidate_projections=(changed_projection, *fragment.candidate_projections[1:]),
        )
        changed_fragments = (changed_fragment, *envelope.fragment_projections[1:])
    ...
    def test_recomputed_unit8_hash_cannot_hide_source_record_factorization_tamper(self):
        envelope = self.synthetic_envelope
        changed = replace(
    ...
    def test_tampered_unit7_envelope_fails_full_replay_before_unit8_projection(self):
        ...
        result = BaziClassicalResolutionEffectDispositionEngine().resolve_typed(changed_request)
    ...
    def test_changed_resolution_mapping_and_closure_status_fail_closed(self):
        candidate = unit8_tests.BaziClassicalResolutionEffectDispositionR1Tests.resolution_candidate
        bad_mechanism = replace(
            candidate,
            mechanism_proposal_kind="RESOLUTION_FAILURE_MECHANISM_PROPOSAL",
        )
        with self.assertRaises(ValueError):
            unit8_tests.expected_candidate_projection(bad_mechanism, self.profile)
        ...
        with self.assertRaises(ValueError):
            unit8_tests.expected_candidate_projection(bad_closure, self.profile)
    ...
    def test_duplicate_unit7_outer_lineage_fails_closed(self):
        ...
        result = BaziClassicalResolutionEffectDispositionEngine().resolve_typed(
