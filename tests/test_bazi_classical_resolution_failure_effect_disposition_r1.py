...
from fortune_training.bazi_classical_resolution_failure_effect_disposition.release import (
...
class BaziClassicalResolutionFailureEffectDispositionR1Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.cross = cls._unit9(source.cross)
        cls.multiplicity = cls._unit9(source.multiplicity)
        cls.base_allocation_candidate = source.controlled_fragment.final_candidates[0]
        cls.failure_candidate = cls._failure_candidate(cls.base_allocation_candidate)

    @classmethod
    def _unit9(cls, stack):
        effect, admission, semantic, mechanism, allocation, _, final = stack
        request = BaziClassicalResolutionFailureEffectDispositionRequest(
            effect,
            admission,
            semantic,
            mechanism,
    ...
    @staticmethod
    def _failure_candidate(candidate):
        closure = ClassicalSemanticClosureGovernanceRow(
            closure_requirement_id="CLASSICAL_RESOLUTION_FAILURE_SEMANTICS",
        ...
        return replace(
            candidate,
            final_candidate_id="CONTROLLED-UNIT9-RESOLUTION-FAILURE-FINAL-CANDIDATE",
            source_claim_edge_class="SOURCE_ASSERTED_RESOLUTION_FAILURE",
            source_assertion_class="RESOLUTION_FAILURE_ASSERTION",
            effect_facet="RELATION_EFFECT_DISPOSITION",
            semantic_candidate_kind="SOURCE_GROUNDED_RESOLUTION_FAILURE_CANDIDATE",
            mechanism_proposal_kind="RESOLUTION_FAILURE_MECHANISM_PROPOSAL",
    ...
    def test_release_contract_is_exact_and_closed(self):
        ...
        self.assertEqual(
            "6f2a7b15db1d97a75056c7ad28e489c26a07aa28ba30b6a9644c8be5d2aff583",
            report["contract_semantics_sha256"],
        )
        contract = json.loads(
            (
                ROOT
                / "audits/bazi-classical-resolution-failure-effect-disposition-r1/contract.json"
        ...
        changed["local_resolution_failure_closure_contract"][
            "reversal_reappearance_released"
        ] = True
    ...
    def test_failure_candidate_projects_one_candidate_local_disposition(self):
        projection = expected_candidate_projection(self.failure_candidate, self.profile)
        self.assertEqual(
            "RESOLUTION_FAILURE_EFFECT_DISPOSITION_PROJECTED",
            projection.projection_status,
        )
        self.assertEqual(self.failure_candidate, projection.source_final_candidate)
        self.assertEqual(1, len(projection.resolution_failure_closure_rows))
        local = projection.resolution_failure_closure_rows[0]
        self.assertEqual(
            "CLASSICAL_RESOLUTION_FAILURE_SEMANTICS",
        ...
        self.assertEqual(
            "AVAILABLE_EXACTLY_AS_CANDIDATE_LOCAL_SOURCE_ASSERTED_FAILED_RESOLUTION_EFFECT_DISPOSITION",
            local.unit9_local_closure_result,
        )
        self.assertEqual(1, len(projection.resolution_failure_effect_dispositions))
        disposition = projection.resolution_failure_effect_dispositions[0]
        self.assertEqual(
            "SOURCE_ASSERTED_FAILED_RESOLUTION_EFFECT_DISPOSITION",
            disposition.disposition_kind,
        )
        self.assertEqual(
            "SOURCE_ASSERTED_NOT_HOLDING_AS_RESOLUTION",
            disposition.resolution_mechanism_disposition,
        )
        self.assertEqual(
            "CANDIDATE_LOCAL_SOURCE_ASSERTED_EFFECT_BRANCH_ONLY",
            disposition.semantic_scope,
        )
        ...
        self.assertEqual(
            self.failure_candidate.target_exact_relation_id,
            disposition.target_exact_relation_id,
        )
        self.assertEqual(
            self.failure_candidate.target_effect_channel_id,
            disposition.target_effect_channel_id,
        )
        self.assertEqual(
            "MISSING_PRIMITIVE",
            self.failure_candidate.closure_governance_rows[0].runtime_dependency_status,
        )

    def test_non_failure_candidates_emit_zero_failure_dispositions(self):
        variants = (
        ...
        for semantic_kind, mechanism_kind, claim_class, facet in variants:
            ...
            self.assertEqual(
                "PRESERVED_NON_RESOLUTION_FAILURE_CANDIDATE",
                projection.projection_status,
            )
            self.assertEqual((), projection.resolution_failure_closure_rows)
            self.assertEqual((), projection.resolution_failure_effect_dispositions)
            self.assertEqual((), projection.resolution_failure_effect_disposition_ids)

    def test_same_channel_failure_candidates_remain_separate_and_unranked(self):
        first = expected_candidate_projection(self.failure_candidate, self.profile)
        second_candidate = replace(
            self.failure_candidate,
            final_candidate_id="CONTROLLED-UNIT9-RESOLUTION-FAILURE-FINAL-CANDIDATE-SECOND",
            source_claim_edge_id=f"{self.failure_candidate.source_claim_edge_id}:SECOND",
        )
        ...
        self.assertEqual(
            ...
            row.candidate_projection_ids,
        )
        self.assertEqual(
            "IDENTITY_ONLY_NO_MERGE_RANK_ARBITRATION_SELECTION_OR_GLOBAL_STATE",
            row.index_semantics,
        )
        self.assertEqual(2, len(row.resolution_failure_effect_disposition_ids))
    ...
    def test_zero_candidate_fragment_remains_zero_candidate(self):
        ...
        self.assertEqual((), projected.resolution_failure_effect_disposition_ids)
        ...
        for request, result in (self.cross, self.multiplicity):
            ...
            for source_outer, projected_outer in zip(
                ...
                for source_fragment, projected_fragment in zip(
                    ...
                    self.assertEqual(
                        source_fragment.final_fragment_id,
                        projected_fragment.source_final_fragment_id,
                    )
                    self.assertEqual(
                        source_fragment.final_candidate_ids,
                        projected_fragment.source_final_candidate_ids,
                    )
                self.assertEqual(
                    "SOURCE_ASSERTED_NOT_HOLDING_AS_RESOLUTION",
                    projected_outer.resolution_mechanism_disposition,
                )
                self.assertEqual(
                    "NOT_RELEASED",
                    projected_outer.target_relation_restored_state_semantics,
                )
                self.assertEqual(
                    "NOT_RELEASED", projected_outer.reversal_reappearance_semantics
                )
                self.assertEqual(
                    "NOT_RELEASED",
                    projected_outer.interaction_chain_execution_semantics,
                )
                self.assertEqual(
                    "NOT_RELEASED", projected_outer.candidate_global_truth_semantics
                )
                self.assertEqual(
                    "NOT_RELEASED", projected_outer.global_operability_semantics
                )
                self.assertEqual(
                    "NOT_RELEASED", projected_outer.execution_readiness_semantics
                )
                self.assertEqual(
                    "NOT_RELEASED", projected_outer.final_classical_verdict_semantics
                )

    def test_runtime_schema_rejects_global_and_restoration_surface(self):
        payload = BaziClassicalResolutionFailureEffectDispositionEngine().resolve(
            self.cross[0]
        )
        schema = json.loads(
            (
        ...
        forbidden = [
            "restored", "reactivated", "active", "in_force", "reappeared",
