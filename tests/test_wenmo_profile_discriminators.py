...
class WenmoProfileDiscriminatorTests(unittest.TestCase):
    ...
    def _birth(self, case: dict) -> BirthInput:
        return BirthInput(
    ...
    def _resolve(self, case: dict):
        return self.engine.resolve(ZiweiChartRequest(birth=self._birth(case), sex=Sex.MALE, profile=self.wenmo_profile))
    ...
    def test_xin_year_kui_yue_is_profile_discriminating_not_a_global_override(self):
        ...
        wenmo_kui, wenmo_yue = WenmoDefaultCoreAuxiliaryGenerator.kui_yue("辛")
        qs_kui, qs_yue = QSCoreAuxiliaryGenerator.kui_yue("辛")
        self.assertEqual(("寅", "午"), (wenmo_kui.address.branch, wenmo_yue.address.branch))
        expected_qs = case["strict_qs_kui_yue_counterfactual"]
        self.assertEqual((expected_qs["STAR.TIANKUI"], expected_qs["STAR.TIANYUE"]), (qs_kui.address.branch, qs_yue.address.branch))
        self.assertNotEqual((wenmo_kui.address.branch, wenmo_yue.address.branch), (qs_kui.address.branch, qs_yue.address.branch))

    def test_1991_fixture_exposes_historical_time_policy_difference_instead_of_redefining_time(self):
        case = self._case("WENMO-CHARTDIFF-005")
        time_result = self.time_calendar.resolve(self._birth(case), self.wenmo_policies)
        branch = time_result["branches"][0]
