...
class HistoricalProvenanceAuditMatrixR1Test(unittest.TestCase):
    ...
    def test_machine_gate_runs(self) -> None:
        ...
        self.assertGreaterEqual(receipt["confirmed_provenance_metadata_defect_count"], 11)
        self.assertGreaterEqual(receipt["repaired_provenance_metadata_defect_count"], 11)
    ...
    def test_batch_12ao_jielan_complete_public_surface_stays_non_physical_and_fail_closed(self) -> None:
        row = next(row for row in self.rows if row["rule_id"] == "HPA-ZDATE-006")
        self.assertEqual(row["batch_12ao_jielan_inclement_time_acquisition_artifact"], "docs/research/ZIWEI-JIELAN-INCLEMENT-TIME-ACQUISITION-R1.json")
        ...
        self.assertEqual(row["runtime_time_standard_binding_status_batch_12ao"], "EARLY_1581_JIELAN_COMPLETE_PUBLIC_TRANSCRIPTION_NONATTESTATION_CONFIRMED_FULLBOOK_INCLEMENT_LUOJING_CLAUSE_REMAINS_SOURCE_SCOPED_AND_CLOCK_INPUT_UNRESOLVED")
        self.assertEqual(row["batch_12ao_new_early_ziwei_transcription_variant_dimension_current_status"], "RETRACTED_BY_BATCH_12AP_AS_OVER_BROAD_SURFACE_EXTRAPOLATION")
        self.assertEqual(row["audit_status"], "MISSING_FROM_PRODUCT")
        self.assertFalse(row["algorithm_reopen_authorized"])



    def test_batch_12ap_repairs_ao_scope_without_runtime_reopen(self) -> None:
        row = next(row for row in self.rows if row["rule_id"] == "HPA-ZDATE-006")
        self.assertEqual(row["batch_12ap_jielan_scope_correction_artifact"], "docs/research/ZIWEI-JIELAN-BIRTH-TIME-CHAPTER-SCOPE-CORRECTION-R1.json")
        self.assertEqual(row["batch_12ap_prov_defect_id"], "PROV-DEFECT-011")
        self.assertEqual(row["batch_12ap_prov_defect_type"], "EVIDENCE_SCOPE_MISCLASSIFICATION")
        self.assertEqual(row["batch_12ap_repair_status"], "REPAIRED_FORWARD_ONLY_DURING_BATCH_12AP")
        self.assertTrue(row["batch_12ap_ao_tianji_pagination_nonattestation_scope_preserved"])
        self.assertTrue(row["batch_12ap_ao_early_transmission_variant_inference_retracted"])
        self.assertEqual(row["batch_12ap_google_books_volume_id"], "rZRcCwAAQBAJ")
        self.assertTrue(row["batch_12ap_google_books_pt49_inclement_index_attested"])
        self.assertEqual(row["batch_12ap_google_books_pt49_heading_index_attested"], "論十二生時難定訣")
        self.assertFalse(row["batch_12ap_google_books_pt49_physical_glyph_authority"])
        self.assertTrue(row["batch_12ap_ncc_genuine_facsimile_spreads_directly_reviewed"])
        self.assertFalse(row["batch_12ap_ncc_pt49_target_page_observed"])
        self.assertFalse(row["batch_12ap_fullbook_inclement_current_time_acquisition_mechanism_closed"])
        self.assertFalse(row["batch_12ap_runtime_standard_selected"])
        self.assertFalse(row["candidate_selected_batch_12ap"])
